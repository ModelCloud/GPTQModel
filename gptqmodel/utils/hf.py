...
def ensure_qwen_drive_registered(model_id_or_path: Optional[str] = None) -> bool:
    """Register the optional Qwen-Drive config before AutoConfig reads it."""

    if _local_config_model_type(model_id_or_path) != _QWEN_DRIVE_MODEL_TYPE:
        return False
    try:
...
def _patch_transformers_prism_gguf_compat(*, api_name: str) -> None:
    ...
    with _MONKEY_PATCH_LOCK:
        ...
        log.warning(
            ...
            "GGUF runtime and local Prism compatibility patch for `%s`.",
            api_name,
        )
