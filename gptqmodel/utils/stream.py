...
def _resolve_device_index(device: torch.device) -> int:
    ...
    if backend is not None and hasattr(backend, "current_device"):
...
def _new_event(device_type: str) -> Any:
    ...
    if backend is None:
        return None
    event_cls = getattr(backend, "Event", None)
    if event_cls is None:
        return None
    try:
        return event_cls(enable_timing=False, blocking=False)
    except TypeError:
        return event_cls(enable_timing=False)
...
def _queue_key_for_device(device: Optional[torch.device]) -> str:
    ...
    if dev_type == "npu" and hasattr(torch, "npu"):
        index = device.index
        if index is None and torch.npu.is_available():
            try:
