...
def _iter_dynamic_contracts(
    ...
    mixed-bitwidth model include kernels such as `MarlinLinear` for 4-bit layers even
    when the base `bits` is 3.
    """
...
def select_quant_linear(
    ...
    if backend in [BACKEND.AUTO, BACKEND.AUTO_TRAINABLE]:
        ...
        # mixed-bitwidth models pick different kernels per layer, e.g. Marlin for 4-bit
        # layers while the base bits is 3. For single-select we keep the original base
        # contract with the full dynamic map so callers asking for one representative
        # kernel get a model-wide-compatible answer.
        if multi_select:
