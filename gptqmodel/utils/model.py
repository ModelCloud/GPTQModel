...
def find_modules(module: nn.Module, layers=None, name: str="") -> Dict[str, nn.Module]:
    ...
    if isinstance(module, tuple(layers)) or isinstance(module, BaseQuantLinear):
       return {name: module}
...
def convert_gptq_v1_to_v2_format(
    ...
    for _, submodule in model.named_modules():
        ...
        # v1 checkpoint format with sym=False saved via convert_gptq_v2_to_v1_format() will
        # overflow ~<=13% based on testing
        if isinstance(submodule, BaseQuantLinear) and submodule.REQUIRES_FORMAT_V2:
    ...
    if not converted:
        log.info(f"Format: Skipped v1 to v2 conversion; no selected kernel requires v2 (`{qlinear_kernel}`).")
...
def untie_word_embeddings(model: nn.Module) -> nn.Module:
    ...
    new_head = nn.Linear(hidden_size, vocab_size, bias=False).to(input_embeddings.weight.device)
    new_head.weight.data = input_embeddings.weight.data.clone()
