...
def _swordfish_static_runtime_error() -> str:
    ...
    if torch_major_minor < _SWORDFISH_TORCH_STABLE_ABI_TARGET:
        return (
            f"Swordfish kernel requires torch >= {'.'.join(map(str, _SWORDFISH_TORCH_STABLE_ABI_TARGET))} "
            f"(compiled against the torch {'.'.join(map(str, _SWORDFISH_TORCH_STABLE_ABI_TARGET))} stable ABI); "
