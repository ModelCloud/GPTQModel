...
_PYTHON_ABI_USAGE_PATTERN = pcre.compile(
    r"\b(?:Py[A-Z]\w*|PYBIND11_\w+)\b|\bpy::"
)
_REGISTER_EXTENSION_PATTERN = pcre.compile(
    r"(?m)\bREGISTER_EXTENSION\s*\("
)
    ...
    def _cache_fingerprint(
        ...
        *,
        ...
        extra_cflags = (
            ...
            else self._resolved_extra_cflags()
        )
        extra_cuda_cflags = (
        ...
        extra_cflags = self._with_stable_abi_target(extra_cflags)
        extra_cuda_cflags = self._with_stable_abi_target(extra_cuda_cflags)
        include_paths = (
