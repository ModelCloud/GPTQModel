...
class AWQProcessor(LoopProcessor):
    ...
    def record_moe_root_input_feature(self, module_name: str, feature: torch.Tensor) -> None:
        ...
        relative_root = module_name.rsplit(".", 1)[-1]
        with self.lock:
            # ModuleLooper may expose subset keys relative to the layer while
            # the lifecycle callback receives the full model path.  Accept
            # either spelling so root capture does not silently miss a layer.
            if not any(
                name == module_name
                or name.startswith(task_prefix)
                or name.startswith(f"{relative_root}.")
                or f".{module_name}." in f".{name}."
