...
from ..utils.model import find_modules, get_module, get_module_by_name_prefix, move_to, MoETopKState, set_moe_topk, restore_moe_topk, untie_word_embeddings
...
class ModuleLooper():
    ...
    def _loop_impl(self, fallback=None, **kwargs):
        ...
        if self.embed_quant_mode is not None:
            if self.embed_quant_mode in (QuantizeEmbed.INPUT, QuantizeEmbed.BOTH):
                if self.input_embeddings_module is None or self.input_embeddings_name is None:
                    raise ValueError("could not find input embeddings module in the model, exit...")
                if not isinstance(self.input_embeddings_module, tuple(SUPPORTS_MODULE_TYPES)):
                    raise NotImplementedError(
                        f"This type({type(self.input_embeddings_module)}) of input embeddings quantization is currently not "
            ...
            if self.embed_quant_mode in (QuantizeEmbed.OUTPUT, QuantizeEmbed.BOTH):
                if self.output_embeddings_module is None or self.output_embeddings_name is None:
                    raise ValueError("could not find output embeddings module in the model, exit...")
                if not isinstance(self.output_embeddings_module, tuple(SUPPORTS_MODULE_TYPES)):
                    raise NotImplementedError(
                        f"This type({type(self.output_embeddings_module)}) of output embeddings quantization is currently not "
                        f"supported. SUPPORTS_MODULE_TYPES is {SUPPORTS_MODULE_TYPES}"
                    )
    ...
    def hook_embeddings_module(self, module):
        if module and isinstance(module, torch.nn.Linear):
            ...
            module_path = (self.output_embeddings_name or self.gptq_model.lm_head).split('.')
