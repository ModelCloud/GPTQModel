...
from ..utils.torch import (CPU, META, torch_sync, tf32_high_precision_guard)
from .. import DEVICE_THREAD_POOL
from .awq_processor import AWQProcessor
from .qqq_processor import QQQProcessor
from .lifecycle import InputCaptureStage, LayerLoopStage
...
class ModuleLooper():
    ...
    def cache_inputs(self, layers, calibration_data, use_cache):
        return InputCaptureStage(self, logger=log).capture(
            layers=layers,
    ...
    def _loop_impl(self, fail_safe: bool = False, **kwargs):
        return LayerLoopStage(self, logger=log).run(
            fail_safe=fail_safe,
            **kwargs,
        )
