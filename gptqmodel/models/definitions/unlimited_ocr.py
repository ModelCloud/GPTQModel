...
from ...quantization.config import LengthAwareConfig, LengthAwareMode
...
class UnlimitedOCRQModel(BaseQModel):
    ...
    def pre_quantize_generate_hook_start(self):
        ...
        materialize_device = normalize_device_like(self.quantize_config.device) or CPU
        self.shell_module_materialize(core_model.embed_tokens, materialize_device)
        self.shell_module_materialize(core_model.norm, materialize_device)
        self.shell_module_materialize(core_model.sam_model, materialize_device)
        self.shell_module_materialize(core_model.vision_model, materialize_device)
        self.shell_module_materialize(core_model.projector, materialize_device)
        self._restore_vision_position_ids(core_model)
        self._restore_text_rotary_embeddings(core_model)
        self._move_direct_parameters(materialize_device)
    ...
    def after_model_load(self, model, load_quantized_model=False):
        ...
        self._restore_vision_position_ids(model.model)
        self._restore_text_rotary_embeddings(model.model)
        return model
    ...
    @staticmethod
    def _restore_text_rotary_embeddings(core_model) -> None:
        """Rebuild checkpoint-omitted Llama RoPE buffers after lazy materialization."""
        ...
        for layer in layers:
            ...
            try:
                template = type(rotary)(config=config)
            except (TypeError, ValueError):
                continue

            current_inv_freq = getattr(rotary, "inv_freq", None)
            if (
                torch.is_tensor(current_inv_freq)
                and current_inv_freq.device.type != "meta"
            ):
                target_device = current_inv_freq.device
            else:
                first_parameter = next(attention.parameters(), None)
                parameter_device = getattr(first_parameter, "device", None)
                target_device = (
                    parameter_device
                    if parameter_device is not None and parameter_device.type != "meta"
                    else CPU
                )

            for name in ("inv_freq", "original_inv_freq"):
                source = getattr(template, name, None)
                if not torch.is_tensor(source):
                    continue
                current = getattr(rotary, name, None)
                target_dtype = current.dtype if torch.is_tensor(current) else source.dtype
                restored = source.to(device=target_device, dtype=target_dtype)
                if name in rotary._buffers:
                    rotary._buffers[name] = restored
                    rotary._non_persistent_buffers_set.add(name)
                else:
                    if hasattr(rotary, name):
                        delattr(rotary, name)
                    rotary.register_buffer(name, restored, persistent=False)
            ...
            for name in (
                ...
                if hasattr(template, name):
                    setattr(rotary, name, getattr(template, name))
    ...
    def prepare_dataset(self, calibration_dataset, batch_size: int = 1, **kwargs):
        ...
        self._materialize_image_length_aware_hessian(calibration_data)
        return calibration_data

    def _materialize_image_length_aware_hessian(self, calibration_data) -> None:
        """Resolve feasible Hessian buckets from the checkpoint's image-token layout."""

        hessian = getattr(self.quantize_config, "hessian", None)
        length_aware = getattr(hessian, "length_aware", None)
        if (
            not isinstance(length_aware, LengthAwareConfig)
            or length_aware.mode is not LengthAwareMode.EQUAL_PER_BUCKET_WEIGHT
            or length_aware.bucket_boundaries is not None
        ):
            return
        ...
        for sample in calibration_data:
            ...
            lengths.extend(
                int(row.sum().item())
                for row in attention_mask.reshape(-1, attention_mask.shape[-1])
            )
        if not lengths:
            return

        hessian.length_aware = LengthAwareConfig.from_lengths(
            lengths,
            mode=length_aware.mode,
            min_length=length_aware.min_length,
            min_bucket_size=length_aware.min_bucket_size,
            max_bucket_ratio=length_aware.max_bucket_ratio,
            bucket_weight_exponent=length_aware.bucket_weight_exponent,
            target_bucket_count=None,
        )
