...
_KOMODO_CANN_PREFETCH_ENV = "GPTQMODEL_KOMODO_CANN_PREFETCH"
_KOMODO_CANN_PREFETCH_MAX_BYTES_ENV = "GPTQMODEL_KOMODO_CANN_PREFETCH_MAX_BYTES"
_KOMODO_CANN_PREFETCH_MIN_BYTES_ENV = "GPTQMODEL_KOMODO_CANN_PREFETCH_MIN_BYTES"
...
def _try_load_komodo_cann_v3() -> bool:
    global _FUSED_OP_LAST_ERROR

    if not _komodo_cann_v3_enabled():
        return False
    try:
        from ...utils.komodo_cann import komodo_cann_v3_runtime_error, load_komodo_cann_v3

        loaded = load_komodo_cann_v3()
        _FUSED_OP_LAST_ERROR = "" if loaded else komodo_cann_v3_runtime_error()
        return loaded
...
def _komodo_cann_fused_status(group_size: int) -> tuple[bool, bool, bool, str | None, str]:
    enabled = _komodo_cann_fused_enabled()
...
def _komodo_cann_base_n(rows: int, in_features: int, out_features: int, cube_consumer_requested: bool) -> int:
...
def _komodo_cann_tiling_plan(
    *,
    ...
    base_n = _komodo_cann_base_n(rows, in_features, out_features, cube_consumer_requested)
    ...
    prefetch_max_bytes = _komodo_cann_prefetch_max_bytes(device)
...
def komodo_cann_plan_asdict(plan: KomodoCannTilingPlan | None) -> dict | None:
    if plan is None:
        return None
    return asdict(plan)
...
def _komodo_cann_fused_matmul(
    *,
    plan: KomodoCannTilingPlan,
    x: torch.Tensor,
...
def _komodo_cann_weight_quant_matmul(
    *,
    plan: KomodoCannTilingPlan,
    x: torch.Tensor,
    weight: torch.Tensor,
...
class _KomodoCannPlanMixin:
    ...
    def _cann_plan(
        ...
        if cached is None:
            cached = _komodo_cann_tiling_plan(
                rows=x_flat.shape[0],
...
class KomodoCannLinear(_KomodoCannPlanMixin, KomodoLinear):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._cann_hot_plan_key = None
        self._cann_hot_plan = None
        self._last_cann_plan: KomodoCannTilingPlan | None = None
    ...
    def _native_forward(self, x: torch.Tensor):
        ...
        if out is None:
            ...
            if plan.prefetch_enabled:
                _komodo_cann_prefetch(plan, x_flat, packed_weight, scales, offsets, bias if fuse_bias else None)
            out = _komodo_cann_weight_quant_matmul(
                plan=plan,
                x=x_flat,
    ...
    def _native_group16_forward(self, x: torch.Tensor):
        ...
        if plan.prefetch_enabled:
            _komodo_cann_prefetch(plan, x_flat, packed_stack, scale_stack, offset_stack, bias_stack)
        self._maybe_schedule_lookahead(compute_dtype)
...
class AwqKomodoCannLinear(_KomodoCannPlanMixin, AwqKomodoLinear):
    ...
    def _native_forward(self, x: torch.Tensor):
        ...
        output = _komodo_cann_fused_matmul(
            plan=plan,
            x=x_flat,
            packed_weight=packed_weight,
            scales=scales,
        ...
        if output is None:
            self._last_cann_path = "native_weight_quant_batchmatmul"
            if plan.prefetch_enabled:
                _komodo_cann_prefetch(plan, x_flat, packed_weight, scales, offsets)
            output = _komodo_cann_weight_quant_matmul(
                plan=plan,
                x=x_flat,
                weight=packed_weight,
                scales=scales,
