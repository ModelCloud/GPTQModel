...
class QQQLinear(GroupedQuantLinear):
    ...
    def pack(
        ...
        Weight packing is chunked to bound the temporary activation-sized tensor.  The
        optional ``workers`` argument is retained for QVQ's CPU packing extension.
        """
        if self.group_size != self.in_features:
        ...
        for input_start in range(0, self.in_features, input_chunk_size):
            ...
            for output_start in range(0, self.out_features, output_chunk_size):
                ...
                # The native QVQ packer is CPU-only; keep the temporary bounded to
                # one input/output tile and fall back to eager bit packing when the
                # optional extension is unavailable.
                transformed_u4 = (transformed & 0xF).to(CPU)
                try:
                    from .pack_block_ext import pack_qqq_cpu

                    packed_chunk = pack_qqq_cpu(transformed_u4, self.bits, qqq_threads)
                except Exception:
                    ...
                    for lane in range(8):
                        packed_chunk.bitwise_or_(
                            (transformed_u4[:, lane::8] & 0xF) << (4 * lane)
                        )
...
class QQQTorchLinear(QQQLinear):
    ...
    def _dequantize_weight_for_torch(self, *, device: torch.device | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if weight.device != target_device or weight.dtype != target_dtype or not weight.is_contiguous():
            weight = weight.to(device=target_device, dtype=target_dtype).contiguous()
    ...
    def forward(self, A):
        ...
        quant_A, s1 = self.dynamic_quant(A)
        weight, s_channel = self._dequantize_weight_for_torch(device=A.device)
        accum = torch.matmul(quant_A.to(torch.float32), weight)
        accum = accum * s1
        accum = accum * s_channel
