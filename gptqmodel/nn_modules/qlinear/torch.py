...
def _right_shift_unpack(values: torch.Tensor, shifts: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
    ...
    if device.type == "npu":
        # torch-npu 2.9 currently mishandles broadcasted bitwise shifts used
        # for low-bit unpacking. Keep unpacking correct, then continue dequant
        # and matmul on NPU.
        values = values.to("cpu")
        shifts = shifts.to("cpu")

    unpacked = torch.bitwise_right_shift(values, shifts).to(dtype)
    if device.type == "npu":
