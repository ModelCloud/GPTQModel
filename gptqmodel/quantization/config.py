...
META_QUANTIZER_GTPQMODEL = "gtpqmodel"
...
class QuantizeConfig():
    ...
    def is_quantized_or_packed_by_v2(self) -> bool:
        ...
        by_v2 = (producer == META_QUANTIZER_GTPQMODEL) and (version.parse(_version) >= version.parse(MIN_VERSION_WITH_V2))

        # fallback to meta.packer
        if not by_v2:
            producer, _version = self.meta_get_versionable(META_FIELD_PACKER)
            by_v2 = producer == META_QUANTIZER_GTPQMODEL and version.parse(_version) >= version.parse(
                MIN_VERSION_WITH_V2
            )
