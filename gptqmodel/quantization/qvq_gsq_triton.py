...
@triton.jit
def _fp32_fma(left, right, addend):
    return tl.inline_asm_elementwise(
        "fma.rn.f32 $0, $1, $2, $3;", "=f,f,f,f", [left, right, addend],
        dtype=tl.float32, is_pure=True, pack=1,
    )
...
def lion_update(parameter, gradient, momentum, *, learning_rate: float,
    ...
    if any(value.dtype != torch.float32 or not value.is_cuda
           ...
           for value in (parameter, gradient, momentum)):
        raise ValueError("fused GSQ Lion requires contiguous CUDA FP32 tensors")
