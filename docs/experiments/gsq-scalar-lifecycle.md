...
| ParoQuant | Optional module-scope GSQ config and processor binding; frozen exported rotations, AWQ packing, CPU checks and eight SM80 native reload/graph cases; real QKV baseline/fixed/learned arms pass 288 native checks with identical payloads ([report](gsq-paro-real-layers.md)); clean-input grouped post-fit binding passes CPU lifecycle checks; grouped native/real-model checks, paired noisy-input binding and full-model propagation pending ([notes](../../research/paroquant.md)) |
