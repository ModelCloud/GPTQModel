...
| ParoQuant | Optional module-scope GSQ config and processor binding; frozen exported rotations, AWQ packing, CPU checks and eight SM80 native reload/graph cases; real QKV baseline/fixed/learned arms pass 288 native checks with identical payloads ([report](gsq-paro-real-layers.md)); grouped-scope binding and full-model propagation pending ([notes](../../research/paroquant.md)) |
