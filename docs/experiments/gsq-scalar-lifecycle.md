...
| ParoQuant | Optional module-scope GSQ config and processor binding; frozen exported rotations, AWQ packing, CPU checks and eight SM80 native reload/graph cases; real QKV baseline/fixed/learned arms pass 288 native checks with identical payloads ([report](gsq-paro-real-layers.md)); clean-input grouped post-fit binding passes CPU lifecycle checks; real layer-0 grouped pilots pass 576 native checks with fixed-GSQ decoder-MSE regressions ([report](gsq-paro-real-group.md)); later-layer paired noisy-input validation and full-model propagation pending ([notes](../../research/paroquant.md)) |
