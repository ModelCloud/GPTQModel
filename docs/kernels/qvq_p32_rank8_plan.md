...
| 1 | Normal P32 → lossless window → original-teacher output residual → two output-aware fits in one job, document-disjoint validation | Public `quantize(rank8_capture=...)` for materialized or explicitly lazy teachers; bounded document capture, module finalizer and tiny-Llama integration | Bounded scalable solver, alignment/atomic finalization integration, real-model fit evidence |
