...
| 4 | Existing Hopper WGMMA + optional rank8; H100/H200 BM/BN/warp-group/stage candidates | Existing WGMMA retained; single-projection policies expose BM32/64/128 × BN64/128 with BK256/stages2, while grouped policies expose validated BM32/64/128 × BN64; H200 off/on sweep; selected BN controls the rank8 projection/epilogue warp width and graph-warm identity; composite output-Hadamard widths use the graph-safe fused epilogue when their factored basis is supported, while project-output fusion remains explicitly power-of-two-only | BN32 and additional stage/warp candidates, generic multi-segment grouped BN128, integrate rank8 into consumer pipeline, both-device sweep |
| 5 | Shared input producer, decode producer and consumer; project rank8 alongside WGMMA | Explicit shared SU/H + rank8 producer for power-of-two K; graph-safe composite-Hadamard producer for supported factorizations such as K=5120; folded composite K producer for input_hadamard=false; grouped independent factors tested; graph-safe `concurrent_reference` auxiliary-stream candidate with exact reference arithmetic and shape warm guards | SIMT/half2 vs padded-TC and FP8-factor sweep; resource-aware concurrent scheduling beyond the measured contended stream candidate; B staging/cache sweep |
| 6 | Expansion + FP32 add + output H/SV + final store; defined rounding order | `00a71f4b`: fused Triton expansion/add/H/SV/bias, independent graph-safe reference; folded composite N epilogue for output_hadamard=false; direct FP16 store; prepared FP32 A/B factor residency removes replay conversion allocations; native ABI routes separate/concurrent rank8 through fused FP32-add/FP16-store epilogues for transform-free and power-of-two output-Hadamard modules, including the 17,408-wide transform-free Qwen shape; native project-output epilogue now fuses rank8 projection, expansion, output post-processing and power-of-two Hadamard under explicit fast-only arithmetic gating | Native composite output-Hadamard fusion and full WGMMA producer/consumer fusion |
| 7 | Grouped gate/up and QKV, independent flags, optional SiLU*up, preserve Q/K norm/RoPE/TP | Grouped separate and fused output epilogues tested on H200; fused MLP now carries rank8 through gate/up, SiLU/product and completed FP32 down correction; child-local flags; checkpointed shared-input P32 groups now add each child's rank8 correction in the shared transformed-input domain before child-local output recovery, with graph replay coverage; composite K/N=5120 producer and fused output-Hadamard paths are graph-tested, while unsupported project-output composite fusion is excluded during candidate enumeration | Fused SiLU/down composition beyond the reference path, real-model grouped/TP validation |
| 8 | ZML tunes eligible whole operators over geometry, transforms, quality and grouping; product-specific caches | Python static policy, shape-specific candidate enumeration, strict external controls, correctness-gated native/external executable runner and persistent revalidated caches; hash-manifested native-friendly artifact writer with descriptor-level `payload_sha256` plus typed Zig manifest loader (format/version, names, dtypes, shapes, byte counts, per-file SHA-256 and Python-compatible base/factor semantic hashes checked before upload); prepared native graph-backed ZML off/on custom call with repeated-handle replay; allocation-free `selectFastest` winner selection plus pre-capture `benchmarkExecutable` timing for direct ZML callers; the executable ZML verifier now compiles, warms, validates and measures every BM/BN candidate before capture, records matched correction-off/on overhead, applies an optional manifest-declared recovery budget, and can persist the complete off/on sweep plus selected geometry as versioned JSON bound to `artifact_payload_sha256`; the QvQ-to-ZML fixture exporter now carries that same exact package binding into `manifest.json`; the source adapter now exposes the complete `recovery_kernel`/`recovery_projection` rank8 policy matrix through the 84-byte native ABI, including native project-output fusion, matched off/on geometry, typed non-opaque StableHLO/XLA attributes and arithmetic-signature gating; gfx950 exposes immutable `QVQAMDLaunchConfig` candidates, a pure deterministic selector, and explicit pre-capture `qvq_p32_amd_autotune` timing/cache API; the unified `P32WindowConfig(algorithm="amd_gfx950")`/window tuner forwards each launch choice to the fused consumer, with cold-cache capture rejected; standalone gfx950 rocBLAS now enumerates and times library solutions during preparation, freezes the winner and returns a versioned timing result while rejecting capture; SM80 exposes measured-plus-probe split candidates through `qvq_p32_window_ampere_kernel_candidates`, and grouped SM80/ZML APIs expose independent per-child split tuples, `p32GroupedSelectFastest`, explicit `p32WindowMatmulGroupedWithConfig` launch control, and direct `p32GroupedAutotune` executable measurement; SM90 Python grouping now exposes shape-valid `hopper_m16` child split tuples plus validated BM32/64/128 × BN64 controls and consumes selected geometry/splits in the grouped payload plan; generic multi-segment BN128 is rejected before launch, while the native single-projection tuner retains BN128; native ABI now admits 256-wide transform-free composite dimensions through N17408 while retaining Hadamard guards; Python grouped tuning now exposes full-set shape-aware ordering for direct callers | Non-Hopper grouped candidates, broader shape/device/quality matrix |
| 9 | fast/balanced/quality graphs, mode changes only at request boundaries | Request-owned single-GPU stateless model graphs, transactional three-mode capture, state invalidation, stream ordering and retained payload/output lifetime; tiny and real Llama graph/eager checks; context-managed, idempotent graph-owner cleanup now makes executable lifetime explicit; bounded per-owner LRU retirement synchronizes in-flight work before releasing old graph/pool references; propagation scorecards accept a residency bound and record per-row/final resident-key and retirement telemetry | Stateful generation/KV and TP scheduler integration, measured request overhead/residency on real H100/H200 runs, broader model/shape validation |
...
The six existing single-projection Hopper geometry choices are now externally
selectable, and their H200 off/on sweep is recorded in
`results/p32_window_h200_geometry.json`. Grouped consumers expose the three
...
mode reusing the prepared native graph handle. The source adapter now exposes
the 84-byte recovery policy fields and measures the complete matched off/on
candidate set before selecting a serving graph. This is not the final fused or
TP-aware ZML runtime: explicit nested user capture, stateful KV/TP scheduling
and broader device/shape validation remain open. The
...
The native SM90 ABI now also has a graph-safe fused rank8 epilogue for
power-of-two output Hadamard modules. It folds FP32 base plus FP16 rank8
expansion into the existing mode-3/4 Hadamard rounding contract and final
FP16 store. Composite output widths remain on the reference epilogue until a
matching native transform is certified; candidate enumeration keeps that
boundary explicit.
