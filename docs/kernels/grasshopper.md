...
| 2026-05-22 | `7827034e` | Group128 full-row qrow unroll | Batch GEMM 4/8-bit group128 full-row fixed-group full-tile decode unrolls qweight rows only when `valid_rows == BatchTileRows`. Qwen3-32B `5120x25600` batch8 group128 Nsight improved `223.680 us -> 216.832 us`, registers/thread dropped `48 -> 44`, global reductions stayed at `256000` warp RED instructions, and repeat host median improved `0.2827 ms -> 0.2630 ms`. Adjacent group64 was left unrolled because earlier broad unroll was neutral/slightly worse; final group64 median remained in range at `0.2651 ms`. |
...
| 2026-05-22 | Qwen3-32B row8 group128 after full-row qrow unroll, rank64 dense LoRA | Do not treat this as a projection-sum promotion signal. The targeted `5120x25600` base profile improved, but the fresh full row8 group128 projection sweep measured base `1.1878 ms` vs Marlin `0.7250 ms` (`1.638x`) and LoRA-total `1.5631 ms` vs Marlin `1.4019 ms` (`1.115x`), worse than the previous post-full-tile projection sum. Continue targeting the base dequant/reduction path and split-K global atomics. |
...
| 2026-05-22 | Add `__launch_bounds__(kThreads, 6)` to the batch kernel | Rejected. Qwen3-32B `5120x25600` batch8 group128 median regressed `0.2827 ms -> 0.2892 ms`; forcing more resident CTAs did not offset the register/scheduling constraint cost. |
| 2026-05-22 | Use a full-K direct-store base kernel for wide-output batch8 group128 | Rejected. This removed split-K atomics entirely, but Qwen3-32B `5120x25600` batch8 group128 regressed `0.2630 ms -> 0.4452 ms`. Nsight showed why: global RED traffic dropped `32.77 MB -> 0`, but grid size collapsed `4000 -> 100` CTAs, registers/thread rose `44 -> 62`, active warps dropped `58.37% -> 12.50%`, and kernel time regressed `214.400 us -> 392.736 us`. |
| 2026-05-22 | Split wide-output batch8 group128 into direct first K tile plus atomic tail | Rejected. Removing the output memset and one of forty split-K reduction slices regressed Qwen3-32B `5120x25600` batch8 group128 median `0.2630 ms -> 0.2787 ms`. Nsight measured a `15.840 us` direct-store init kernel plus a `209.600 us` atomic tail, with global RED traffic only dropping `32.77 MB -> 31.95 MB`. |
| 2026-05-22 | Guard fp32-accumulation batch setup to skip input-accumulator initialization | Rejected. Qwen3-32B `5120x25600` batch8 group128 median regressed `0.2630 ms -> 0.2744 ms`; ptxas was already eliminating enough unused low-precision accumulator state that the explicit branch did not help. |
| 2026-05-22 | Use the 4-row batch tile for wide-output group128 base GEMM | Rejected. Qwen3-32B `5120x25600` batch8 group128 median regressed `0.2630 ms -> 0.3049 ms`; the current 8-row tile is still the better tradeoff despite the split-K atomic pressure. |
