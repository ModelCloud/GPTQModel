...
| Q07 higher-rate sweep attempt 4 | telemetry-capable commit `329cc0a5`; exact-auto pruning; all Q07 controls unchanged; W2.5 output `/root/qvq-results/llama32-1b-v2b2p32-yaqa322k-w25-reg010-pruneauto-main-329cc0a5` on physical GPU UUID `GPU-737e2423-874a-23a4-1126-dfbe3e77c294`; W3 output `/root/qvq-results/llama32-1b-v2b2p32-yaqa322k-w30-reg010-pruneauto-main-329cc0a5` on `GPU-724ea08e-67c3-c0ce-29bb-e6c48e7dde28` | running concurrently; each arm independently confirmed 182 YAQA rows / 302,193 tokens and 128 ordinary lifecycle rows / 49,725 tokens; completed modules pass packed round-trip verification. Runtime telemetry reports zero eligible pruning dispatches, zero candidate opportunities/skips, and increasing baseline fallbacks. This is expected from current-main eligibility: Q07's `reselect` mode invokes family-batched grid solves, while exact `norm_band` currently accepts only non-family-batched cells. Therefore pruning is configured but is a correctness-preserving no-op for this exact comparable workflow; quantization continues without changing Q07 controls. Quantization only, no evaluation authorized. |
...
uniform `.20`. Telemetry-capable W2.5 and W3 quantization-only arms are running concurrently. Nothing in this ledger
