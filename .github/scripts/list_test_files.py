...
def getFiles(
    ...
    model_compat_test_files = {
        rel
        for rel, path in all_tests.items()
        if (not input_test_files_list or rel in input_test_files_list)
           and "mlx" not in rel
           and "ipex" not in rel
           and "xpu" not in rel
           and re.match(test_regex, rel)
           and _is_model_compat_test(rel, path)
    }
    ...
    torch_test_files = {
        ...
        if (not input_test_files_list or f in input_test_files_list)
           and f not in model_compat_test_files
           and "mlx" not in f
           and "ipex" not in f
           and "xpu" not in f
    ...
    return (
        sorted(torch_test_files, key=_sort_key),
        sorted(model_compat_test_files, key=_sort_key),
        sorted(m4_test_files, key=_sort_key),
    )
...
def main() -> None:
    ...
    torch_files, model_compat_files, m4_files = getFiles(
    ...
    print(f"{json.dumps(torch_files)}|{json.dumps(model_compat_files)}|{json.dumps(m4_files)}")
