...
def _divergence_metrics(
    ...
    * ``first_divergence_token`` is one-based, with ``token_count + 1`` meaning
    ...
    if dense.shape[0] < token_count:
        return None
    # PyTorch's CUDA argmax is faster than the standalone fused operator for the
    # current 32-token x 128k-vocabulary shape. Keep the native operator
    # available for callers that need compact device-side counters, but use the
    # measured fastest path in the sweep hot loop.
