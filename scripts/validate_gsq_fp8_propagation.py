"""Propagate saved FP8 QKV payloads through the canonical F6 FP32 reference.

Only block-0 QKV use TorchFP8Linear. Other F6 operators use canonical FP32
decoding. Eager, uncached diagnostic; not a production paged/graph comparison
or a complete native FP8 model export.
"""
...
def main():
    ...
    if args.prepare:
        ...
        for name in TARGETS:
            for arm in ('baseline', 'gsq_weight', 'gsq_calibrated'):
        ...
        for path in (Path(__file__), Path('scripts/gsq_f6_reference.py'),
                     Path('scripts/p32_twenty/scorecard.py'), Path('gptqmodel/nn_modules/qlinear/fp8.py')):
        ...
        write_json(args.output / 'provenance.json', {
            ...
            'scope': __doc__, 'format': 'float8_e4m3fn', 'weight_scale_method': 'row',
        })
        return
    ...
    baseline_hashes = []
    for arm in ('f6', 'baseline', 'gsq_weight', 'gsq_calibrated'):
        if arm != 'f6':
            for name in TARGETS:
                ...
                native = TorchFP8Linear(bits=8, group_size=-1, sym=True, desc_act=False,
        ...
        for i, row in enumerate(rows):
            with torch.inference_mode():
                ...
                if arm in ('gsq_weight', 'gsq_calibrated'):
