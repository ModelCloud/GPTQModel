...
"""Compare native grouped Hopper P32 with pre-Phase-3 child launches."""
...
def _physical_h100_uuid() -> str:
    ...
    for line in output.splitlines():
        uuid, name = (field.strip() for field in line.split(",", 1))
        if "H100" in name:
            matches.append((uuid, name))
    if len(matches) != 1:
        raise RuntimeError(f"expected exactly one physical H100, found {matches}")
    return matches[0][0]
...
def _assert_exclusive_h100(torch: Any, expected_uuid: str) -> dict[str, Any]:
    ...
    expected = expected_uuid.removeprefix("GPU-").lower()
    if (
        "H100" not in properties.name
        or (properties.major, properties.minor) != (9, 0)
        or actual_uuid != expected
    ):
        raise RuntimeError(
            f"expected H100 {expected_uuid}, got {properties.name} "
            f"sm_{properties.major}{properties.minor} {properties.uuid}"
        )
    output = subprocess.check_output(
        [
            "nvidia-smi",
...
def _plain_group_call(
    ...
    return tuple(
        ...
        for window, child_selectors, child_n, alt_id in zip(
            windows, selectors, child_ns, bank_alt_ids, strict=True
        )
    )
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    expected_uuid = _physical_h100_uuid()
    visible = os.environ.get("CUDA_VISIBLE_DEVICES", "")
    if visible not in {"1", expected_uuid}:
        raise RuntimeError(
            "set CUDA_VISIBLE_DEVICES to physical H100 index 1 or its UUID"
        )
    ...
    fingerprint = _source_fingerprint(args.repo_root)
    rows = []
    ...
    for bits in args.rates:
        for group_name in args.groups:
            ...
            alt_ids = tuple(int(value) for value in group["bank_alt_ids"])
            generator = torch.Generator(device="cuda").manual_seed(
                20261230 + int(bits * 10) * 100 + list(GROUPS).index(group_name)
            )
            windows = []
            selectors = []
            ...
            for logical_m in args.m_values:
                input_tensor = torch.zeros(
                    (16, size_k), dtype=torch.float16, device="cuda"
                )
                input_tensor[:logical_m] = (
                    torch.randn(
                        (logical_m, size_k),
                        generator=generator,
                        device="cuda",
                ...
                if args.mode == "plain":
                    ...
                    if not all(
                        ...
                        raise RuntimeError(
                            f"grouped output differs for {group_name} "
                            f"M{logical_m} W{bits:g}"
                        )
                    del expected, actual
                    call = grouped_call

                timing, graph, captured = _graph_event_timing(
                    torch,
                    call,
                    warmup=args.warmup,
                ...
                checksum = sum(float(output.sum().item()) for output in captured)
                rows.append(
                    {
                        "bits": bits,
                        "group": group_name,
                ...
                print(
                    f"{args.label}: W{bits:g} {group_name} M={logical_m} "
                    f"K={size_k} N={child_ns} median={timing['median_ms']:.6f} ms",
                    flush=True,
                )
                del graph, captured, input_tensor, payload
                gc.collect()
    ...
    if fingerprint != _source_fingerprint(args.repo_root):
