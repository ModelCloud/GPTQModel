...
"""Compare separate M16 padding with direct padded precondition output on H100."""
...
SOURCE_PATHS = (
    ...
    Path("scripts/benchmark_qvq_phase17_direct_padding.py"),
)
...
def _run(args: argparse.Namespace) -> dict:
    ...
    for m in args.m_values:
        generator = torch.Generator(device="cuda").manual_seed(17000 + m)
        gate = torch.randn(
        ...
        def separate_padding():
            ...
            if m == 16:
                return transformed
            padded = torch.zeros((16, N), dtype=transformed.dtype, device="cuda")
        ...
        if not bit_exact:
            raise RuntimeError(f"direct padding changed M{m} output bits")
        speedup = baseline_timing["median_ms"] / direct_timing["median_ms"]
        eliminated_transient_bytes = 0 if m == 16 else m * N * 2
        row = {
            "m": m,
            "n": N,
            ...
            "bit_exact": bit_exact,
            "eliminated_transient_bytes": eliminated_transient_bytes,
        }
        rows.append(row)
        print(
    ...
    payload = {
        ...
        "workload": "exact N=8192 SwiGLU/down precondition direct-padding experiment",
        "rows": rows,
    }
