...
"""Emit one profiler-bounded paired-recovery launch on H100."""
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("variant", choices=("shared", "warp"))
...
def _run(args: argparse.Namespace) -> None:
    ...
    generator = torch.Generator(device="cuda").manual_seed(91000 + args.m)
    ...
    with torch.inference_mode():
        ...
        if any(
            not torch.equal(result.view(torch.int16), reference.view(torch.int16))
            for result, reference in zip(actual, expected, strict=True)
        ):
            raise RuntimeError("profile controls are not bit-exact")
        use_warp = args.variant == "warp"
        for _ in range(args.warmup):
            profiled_output = call(use_warp)
        torch.cuda.synchronize()
    ...
    if any(
        not torch.equal(result.view(torch.int16), reference.view(torch.int16))
        for result, reference in zip(profiled_output, expected, strict=True)
    ):
