"""Propagate saved staged block-0 weights through canonical F6 FP32.
...
def main():
    ...
    p.add_argument('--packed', action='store_true', help='Also compare packed baseline and staged block 0')
    ...
    if args.prepare:
        ...
        files[str(args.layers/'stages.pt')] = local['payload_sha256']
        for path in (Path(__file__), Path('scripts/gsq_f6_reference.py'), Path('scripts/p32_twenty/scorecard.py'),
                     Path('gptqmodel/quantization/gsq_training.py'), Path('gptqmodel/nn_modules/qlinear/torch.py')):
        ...
        write_json(args.output / 'provenance.json', {
            'layers': str(args.layers), 'files': files, 'seed': 7, 'packed': args.packed,
            'repository_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], text=True).strip(),
            'scope': __doc__, 'format': f"canonical staged scalar W{local['bits']} FP32", 'inputs': str(inputs),
        })
        return
    provenance = json.loads((args.output / 'provenance.json').read_text())
    if (provenance['layers'] != str(args.layers) or provenance.get('packed', False) != args.packed
            or (args.output / 'report.json').exists()):
    ...
    dense_block = copy.deepcopy(model.model.layers[0])
    saved = torch.load(args.layers/'stages.pt', weights_only=True, map_location='cpu')
    report.update(install_f6(model, Path(source['snapshot']) / 'qvq-p32'))
    baseline_hashes = {}
    arms = ['f6', 'baseline', 'staged']
    if args.packed:
        arms.extend(['baseline_packed', 'staged_packed'])
    for arm in arms:
        if arm != 'f6':
            block = copy.deepcopy(dense_block)
            if arm.startswith('staged'):
                block.load_state_dict(saved['state_dict'], strict=True)
            else:
                with torch.no_grad():
                    for name, (weight, _) in saved['initializers'].items():
                        block.get_submodule(name).weight.copy_(weight)
            if arm.endswith('_packed'):
                from gptqmodel.quantization.gsq_training import pack_llama_staged_block

                records = saved['stages']
                if arm == 'baseline_packed':
                    records = {name: {'scales': {'weight': scales}}
                               for name, (_, scales) in saved['initializers'].items()}
                block = pack_llama_staged_block(block, records, bits=local['bits'], group_size=128)
                # Exercise actual serialized payload reload before runtime evaluation.
                payload_path = args.output / f'{arm}.pt'
                torch.save(block.state_dict(), payload_path)
                block.load_state_dict(torch.load(payload_path, weights_only=True), strict=True)
                report.setdefault('packed_payloads', {})[arm] = digest(payload_path)
                block = block.cuda().eval()
            model.model.layers[0] = block
        results = []
        logits_directory = args.output / arm
        ...
        for i, row in enumerate(rows):
            with torch.inference_mode():
                ...
                torch.save(logits.cpu(), logits_directory / f'{i}.pt')
                checksum = hashlib.sha256(logits.cpu().numpy().tobytes()).hexdigest()
                if arm.startswith('baseline'):
                    baseline_hashes.setdefault(arm, []).append(checksum)
                ...
                if arm.startswith('staged'):
                    baseline_arm = arm.replace('staged', 'baseline')
                    metrics['baseline_logits_equal'] = checksum == baseline_hashes[baseline_arm][i]
