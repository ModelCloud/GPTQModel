...
def execute(args):
    ...
    from gptqmodel.quantization import FORMAT, GPTQConfig, GSQConfig, QuantizeConfig, RTNConfig
    ...
    for name in TARGETS:
        ...
        for arm in ARMS:
            ...
            local_rows = []
            for inputs in captures[name]["heldout"]:
                target = inputs @ dense.T
                error = inputs @ canonical.T - target
                local_rows.append({"tokens": len(inputs), "mse": error.square().mean().item(),
                                   "nmse": (error.square().sum()/target.square().sum()).item()})
            packed_objective_target = dense.half().float()
            metric_inputs = x if args.method == "gptq" else torch.eye(in_features, device="cuda")
...
def main():
    ...
    parser.add_argument("--method", choices=("gptq", "rtn"), required=True)
