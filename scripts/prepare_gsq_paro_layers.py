"""Bind real ParoQuant experiments to saved dense weights and disjoint documents.

Preparation only: no quantization or quality claims. The final four calibration
 documents are optimizer validation; the evaluation split remains untouched.
"""
...
def prepare(inputs, output):
    ...
    splits = {"train": documents["train"][:12], "validation": documents["train"][12:],
              "heldout": documents["heldout"]}
    ...
    files.extend(ROOT / path for path in (
        "gptqmodel/quantization/gsq_paro.py", "gptqmodel/quantization/gsq_scalar.py",
        "gptqmodel/quantization/config.py", "gptqmodel/looper/paroquant_processor.py",
        "gptqmodel/quantization/paroquant/optimization.py", "gptqmodel/nn_modules/qlinear/paroquant.py"))
    ...
    manifest = {
        ...
        "files": hashes, "shapes": shapes, "seed": 7, "bits": 4, "group_size": 128, "krot": 8,
        "arms": ["baseline", "gsq_fixed", "gsq_scales"],
        "scope": "real block-0 QKV ParoQuant module optimization and native packed reconstruction",
        "calibration_indices": {"train": list(range(12)), "validation": list(range(12, 16))},
        ...
        "selection": "initializer uses calibration validation; GSQ selects training objective only",
    }
