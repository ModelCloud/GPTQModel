...
"""Compare Phase-63 and paired-output fused recovery on physical H100."""
...
SOURCE_PATHS = (
    ...
    Path("scripts/benchmark_qvq_phase64_paired_recovery_tiles.py"),
)
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("artifacts/a41_phase64_h100/paired_recovery_tiles.json"),
    )
...
def _run(args: argparse.Namespace) -> dict:
    ...
    for m in args.m_values:
        generator = torch.Generator(device="cuda").manual_seed(164000 + m)
        ...
        if not exact:
            raise RuntimeError(f"paired recovery changed M{m} output")
        ...
        rows.append(
            {
                ...
                "speedup_vs_phase63": speedup,
                "better_than_phase63": speedup > 1.0,
                "bit_exact": exact,
                "control_recovery_high_blocks_per_row": 32,
                "candidate_recovery_high_blocks_per_row": 16,
                "control_selected_tree_loads_per_two_outputs": 64,
                "candidate_selected_tree_loads_per_two_outputs": 32,
                "control_selected_tree_rounded_ops_per_two_outputs": 62,
                "candidate_selected_tree_rounded_ops_per_two_outputs": 32,
            }
        )
        print(
            f"M{m}: phase63={control_timing['median_ms'] * 1000:.3f}us "
            f"paired={candidate_timing['median_ms'] * 1000:.3f}us "
