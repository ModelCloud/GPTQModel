#!/usr/bin/env python3
"""Run a single AdjacentExact exact kernel call for ncu profiling."""
...
if __name__ == "__main__":
    device = "cuda:0"
    size = int(sys.argv[1]) if len(sys.argv) > 1 else 28
    problem = make_problem(size, device)
    # One warmup to avoid JIT compile inside the profiled region.
    adjacent_exact_cuda(problem, decompose=False)
