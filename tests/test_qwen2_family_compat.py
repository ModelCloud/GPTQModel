...
def test_resolve_loader_config_keeps_qwen2_5_omni_composite_config_without_text_subconfig():
    class _CompositeConfig:
        sub_configs = {"thinker_config": object}

        def get_text_config(self):
            raise AssertionError("Composite waveform configs must not be collapsed to text configs.")
    ...
    assert model_loader.resolve_loader_config(_WaveformModel, config, trust_remote_code=False) is config


def test_resolve_loader_config_uses_text_config_for_text_only_models():
    class _TextConfig:
        pass
    ...
    class _TextOnlyModel:
        config_class = _TextConfig

    assert model_loader.resolve_loader_config(
        _TextOnlyModel,
        _CompositeConfig(),
