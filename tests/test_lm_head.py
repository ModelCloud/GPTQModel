...
class TestLmHeadQuant(ModelTest):
    ...
    def setUpClass(cls):
        calibration_dataset = \
        load_dataset("json", data_files="/monster/data/model/dataset/c4-train.00000-of-01024.json.gz",
                     split="train").filter(lambda x: len(x["text"]) >= cls.sample_length).select(range(cls.samples))[
            "text"]
