...
def _build_w4a4_module() -> qqq_module.QQQLinear:
    return qqq_module.QQQLinear(
        bits=4,
        group_size=128,
        sym=True,
...
def test_qqq_w4a4_forward_uses_dedicated_jit_kernel(monkeypatch):
    module = _build_w4a4_module()
    calls = {}
    ...
    def fake_gemm(A, B, C, D, s1, s2, s3, workspace, thread_k, thread_n, sms, max_par):
        ...
        D.copy_(torch.full_like(D, 5.0))

    monkeypatch.setattr(qqq_module, "qqq_w4a4_gemm", fake_gemm)

    x = torch.randn((2, module.in_features), dtype=torch.float32)
    out = module(x)

    assert calls["gemm"] == {
        "A_shape": (2, module.in_features // 8),
        "A_dtype": torch.int32,
        "B_shape": tuple(module.B.shape),
        "D_shape": (2, module.out_features),
        "s1_shape": (2, 1),
...
def test_pack_signed_int4_roundtrip_reference_values():
    x = torch.tensor([[-8, -7, -1, 0, 1, 6, 7, 3]], dtype=torch.int8)

    packed = qqq_module.pack_signed_int4(x)

    assert packed.dtype == torch.int32
    assert tuple(packed.shape) == (1, 1)
    assert int(packed[0, 0].item()) == 0x37610F98
