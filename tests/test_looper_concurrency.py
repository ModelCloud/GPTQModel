...
from gptqmodel.looper.loop_processor import LoopProcessor, _SafeDict, _ThreadSafeInputCache
...
def _make_processor() -> LoopProcessor:
    ...
    processor._pb_lock = threading.Lock()
    processor._fwd_time_lock = threading.Lock()
    processor._device_smi_lock = threading.RLock()
    processor._cache_lock = threading.RLock()
    processor._results = {}
    processor.tasks = _SafeDict()
...
def test_loop_processor_initializes_synchronized_state(monkeypatch, tmp_path):
    ...
    assert isinstance(processor.tasks, _SafeDict)
    assert isinstance(processor.inputs_cache, _ThreadSafeInputCache)
    assert isinstance(processor._cache_lock, type(threading.RLock()))
...
def test_safe_dict_iteration_snapshots_during_concurrent_mutation():
    tasks = _SafeDict()
...
def test_safe_dict_mutation_api():
    tasks = _SafeDict()
...
def test_safe_dict_uses_builtin_dict_equality():
    tasks = _SafeDict({"a": 1, "b": 2})
    matching = _SafeDict({"a": 1, "b": 2})

    assert _SafeDict.__eq__ is dict.__eq__
    assert _SafeDict.__ne__ is dict.__ne__
    assert tasks == matching
    assert tasks == {"a": 1, "b": 2}
    different = _SafeDict({"a": 1, "b": 2, "c": 3})
...
def test_eora_progress_uses_synchronized_draw_helper():
    ...
    processor.draw_progress.assert_called_once_with(
        "EoRA: Processing layer.proj (torch.float16) in layer"
    )
