...
"""Benchmark main's serial TQ1_0 packer against parallel MLX block packing."""
...
@lru_cache(maxsize=1)
def _main_tq1_0_kernel():
    return mx.fast.metal_kernel(
        name="gptqmodel_gguf_tq1_0_pack_main_benchmark",
        input_names=["weights"],
        output_names=["packed"],
        source="""
...
def run(samples):
    ...
    for name, rows, columns in QWEN38_27B_PROJECTIONS:
        mx.random.seed(380011 + rows + columns)
        ...
        for label, dtype in (("FP16", mx.float16), ("BF16", mx.bfloat16)):
            ...
            def parallel_fn(value=weight):
                return gguf_quantize_weight_mlx(value, "TQ1_0")
