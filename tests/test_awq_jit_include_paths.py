...
import gptqmodel.utils.awq as awq_module
from gptqmodel.utils import cpp as cpp_module


def test_awq_include_paths_use_wheel_headers_when_local_cuda_is_incomplete(monkeypatch, tmp_path):
    root = tmp_path / "awq"
    ...
    monkeypatch.setattr(awq_module, "_awq_root", lambda: root)
    ...
    include_paths = awq_module._awq_include_paths()

    assert include_paths[0] == str(root)
    assert str(wheel_cuda_include) in include_paths


def test_awq_include_paths_skip_wheel_headers_when_local_cuda_has_required_headers(monkeypatch, tmp_path):
    root = tmp_path / "awq"
    local_cuda_include = tmp_path / "local_cuda_include"
    wheel_cuda_include = tmp_path / "wheel_cuda_include"
    root.mkdir()
