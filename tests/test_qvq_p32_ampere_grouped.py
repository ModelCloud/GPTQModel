...
@pytest.mark.parametrize("size_m", (4, 8, 16), ids=("m4", "m8", "m16"))
@pytest.mark.parametrize(
...
def test_grouped_ampere_rank8_shares_projection_and_matches_child_paths(
    ...
    for width in widths:
        ...
        rank8_as.append(
            (torch.randn((in_features, 8), generator=generator, device=device) * 0.01).half()
        )
        rank8_bs.append(
    ...
    plain = tuple(
        ...
        for window, bank_id, width, alt_id, split_count, rank8_a, rank8_b in zip(
            windows,
            selectors,
            widths,
            alt_ids,
...
def test_flash_next_gate_up_rank8_wide_group_matches_child_paths(size_m, monkeypatch):
    ...
    alt_ids = (3, 1)
    split_counts = (40, 40)
    generator = torch.Generator(device=device).manual_seed(20261218 + size_m)
    levels = pgc16_levels_for_version(PGC16_CODEBOOK_VERSION).contiguous().to(device)
    input = (
        torch.randn((size_m, in_features), generator=generator, device=device) * 0.1
    ).half()
