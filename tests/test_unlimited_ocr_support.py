...
from gptqmodel.quantization.config import HessianConfig, LengthAwareMode
...
def test_unlimited_ocr_materializes_feasible_image_length_aware_hessian():
    qmodel = object.__new__(UnlimitedOCRQModel)
    nn.Module.__init__(qmodel)
    qmodel.quantize_config = SimpleNamespace(hessian=HessianConfig())
    calibration_data = [
        {"attention_mask": torch.ones(1, 277, dtype=torch.long)}
        for _ in range(20)
    ]

    qmodel._materialize_image_length_aware_hessian(calibration_data)

    length_aware = qmodel.quantize_config.hessian.length_aware
    assert length_aware.mode is LengthAwareMode.EQUAL_PER_BUCKET_WEIGHT
    assert length_aware.target_bucket_count is None
    assert length_aware.bucket_boundaries == [0.0, float("inf")]
    assert length_aware.bucket_scales == [277.0]
    assert length_aware.bucket_weights == [1.0]
...
def test_unlimited_ocr_restores_checkpoint_omitted_vision_position_ids():
    ...
    core_model.vision_model.embeddings.register_buffer(
        "position_ids",
    ...
    assert "position_ids" not in embeddings.state_dict()


def test_unlimited_ocr_restores_checkpoint_omitted_text_rotary_buffers():
    config = LlamaConfig(
        hidden_size=16,
    ...
    expected_inv_freq = rotary.inv_freq.clone()
    rotary._buffers["inv_freq"] = torch.zeros_like(rotary.inv_freq)
    rotary._buffers["original_inv_freq"] = torch.zeros_like(
        rotary.original_inv_freq
    )

    attention = nn.Module()
    attention.rotary_emb = rotary
    attention.q_proj = nn.Linear(16, 16, bias=False)
    layer = nn.Module()
    layer.self_attn = attention
    core_model = nn.Module()
    core_model.layers = nn.ModuleList([layer])

    UnlimitedOCRQModel._restore_text_rotary_embeddings(core_model)

    torch.testing.assert_close(rotary.inv_freq, expected_inv_freq)
    torch.testing.assert_close(rotary.original_inv_freq, expected_inv_freq)
    assert {"inv_freq", "original_inv_freq"}.issubset(
        rotary._non_persistent_buffers_set
    )
...
def test_unlimited_ocr_materializes_base_modules_on_concrete_device(monkeypatch):
    model = nn.Module()
    model.model = nn.Module()
    model.model.embed_tokens = nn.Embedding(8, 4)
    model.model.norm = nn.LayerNorm(4)
    ...
    nn.Module.__init__(qmodel)
    qmodel.model = model
    qmodel.quantize_config = SimpleNamespace(device=torch.device("cuda"))

    materialized_devices = []
    monkeypatch.setattr(
        qmodel,
        "shell_module_materialize",
        lambda module, device: materialized_devices.append(device),
    )
    monkeypatch.setattr(qmodel, "_restore_vision_position_ids", lambda core_model: None)
    monkeypatch.setattr(
        qmodel,
        "_move_direct_parameters",
        lambda device: materialized_devices.append(device),
    )

    qmodel.pre_quantize_generate_hook_start()

    assert materialized_devices == [torch.device("cuda:0")] * 6
