...
"""Benchmark main's serial Q4_K packer against parallel MLX block packing."""
...
@lru_cache(maxsize=1)
def _main_q4_k_kernel():
    return mx.fast.metal_kernel(
        name="gptqmodel_gguf_q4_k_pack_main_benchmark",
        input_names=["weights"],
        output_names=["packed"],
        source="""
            ...
            for (uint group = 0; group < 8; group += 2) {
                ...
                for (uint k = 0; k < 32; ++k) {
                    ...
                    uint code1 = step1 > 0.0f
                        ...
                        : 0;
                    packed[offset + 16 + group * 16 + k] = uchar(
                        code0 | (code1 << 4));
                }
            }
        """,
...
def run(samples):
    ...
    for name, rows, columns in QWEN38_27B_PROJECTIONS:
        mx.random.seed(380024 + rows + columns)
        ...
        for label, dtype in (("FP16", mx.float16), ("BF16", mx.bfloat16)):
            ...
            def parallel_fn(value=weight):
                return gguf_quantize_weight_mlx(value, "Q4_K")
