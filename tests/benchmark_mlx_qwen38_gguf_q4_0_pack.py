...
"""Benchmark main's FP32-temporary Q4_0 packer against direct MLX input."""
...
@lru_cache(maxsize=1)
def _main_q4_0_kernel():
    return mx.fast.metal_kernel(
        name="gptqmodel_gguf_q4_0_pack_main_benchmark",
        input_names=["weights"],
        output_names=["packed"],
        source="""
    ...
    for name, rows, columns in QWEN38_27B_PROJECTIONS:
        ...
        for label, dtype in (("FP16", mx.float16), ("BF16", mx.bfloat16)):
            ...
            def direct_fn(value=weight):
                return gguf_quantize_weight_mlx(value, "Q4_0")
