...
def test_yaqa_mlx_symmetric_gram_pack_rejects_asymmetric_input():
    from gptqmodel.utils.qvq_mlx import qvq_mlx_pack_symmetric_gram_from_torch_mps

    asymmetric = torch.tensor([[1.0, 2.0], [3.0, 4.0]], device="mps")
    with pytest.raises(ValueError, match="exactly symmetric"):
        qvq_mlx_pack_symmetric_gram_from_torch_mps(asymmetric)
