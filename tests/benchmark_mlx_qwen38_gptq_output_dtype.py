...
"""Measure GPTQ output-dtype preservation against the merged main path."""
    ...
    for dtype in (mx.float16, mx.bfloat16):
        for name, out_features, in_features in QWEN38_27B_PROJECTIONS:
            linear = nn.QuantizedLinear(
                in_features, out_features, bias=False, group_size=128, bits=4,
            )
            linear.weight = mx.random.randint(0, 2**31, linear.weight.shape).astype(mx.uint32)
            linear.scales = mx.full(linear.scales.shape, 0.001, dtype=mx.float16)
            linear.biases = mx.full(linear.biases.shape, -0.002, dtype=mx.float32)
            wrapped = MlxGPTQLinear(linear)
            for rows in (1, 16):
