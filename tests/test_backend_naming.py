...
def test_legacy_komodo_backend_normalizes_by_quant_method():
    assert normalize_backend(BACKEND.KOMODO, quant_method=METHOD.GPTQ) == BACKEND.GPTQ_KOMODO
...
def test_legacy_komodo_cann_backend_normalizes_by_quant_method():
    assert normalize_backend(BACKEND.KOMODO_CANN, quant_method=METHOD.GPTQ) == BACKEND.GPTQ_KOMODO_CANN
    assert normalize_backend(BACKEND.KOMODO_CANN, quant_method=METHOD.AWQ) == BACKEND.AWQ_KOMODO_CANN
...
def test_name_based_lookup_accepts_canonical_member_names():
    ...
    assert normalize_backend("GPTQ_KOMODO_CANN") == BACKEND.GPTQ_KOMODO_CANN
