...
def _quantize_patched(model_dir: str, dedup: bool, monkeypatch) -> Tuple[List[Tuple[Dict[str, str], List[str]]], Dict[str, torch.Tensor], Dict[str, int]]:
    ...
    qcfg = QuantizeConfig(
        ...
        desc_act=False,
        device="cpu",
        # Keep this tiny fixture focused on dedup semantics; QVQ's adaptive
        # length-aware bucket policy requires a larger calibration corpus.
        hessian=HessianConfig(length_aware=False, dedup_shared_inputs=dedup),
    )
