...
@pytest.mark.parametrize("bounded_rounding", (False, True))
def test_qvq_cuda_fused_recovery_to_precondition_is_bit_exact_and_graph_stable(
    m, scale_mode, pad_to_16, pair_tiles, bounded_rounding
):
    if torch.cuda.get_device_capability()[0] != 9:
...
def test_qvq_cuda_bounded_recovery_rounding_preserves_high_magnitude_fallback():
    if torch.cuda.get_device_capability()[0] != 9:
        pytest.skip("bounded fused recovery requires Hopper")
    m, n = 16, 8192
    generator = torch.Generator(device="cuda").manual_seed(20267067)
    ...
    expected = qvq_cuda_hadamard_pair_swiglu_precondition_multiblock(
        input0,
        input1,
        **shared,
...
def test_qvq_cuda_packed_gate_up_recovery_is_bit_exact_and_graph_stable(
    scale_mode, with_bias
):
    if torch.cuda.get_device_capability()[0] != 9:
        pytest.skip("packed fused recovery requires Hopper")
    m, n = 16, 8192
    generator = torch.Generator(device="cuda").manual_seed(20268068 + scale_mode)
    input0 = torch.randn((m, n), generator=generator, device="cuda") * 20
    input1 = torch.randn((m, n), generator=generator, device="cuda") * 20
    scale0 = torch.randn((n,), generator=generator, device="cuda")
    scale1 = torch.randn((n,), generator=generator, device="cuda")
    ...
    expected = qvq_cuda_hadamard_pair_swiglu_precondition_multiblock(
        input0, input1, **shared
    )
    actual = qvq_cuda_hadamard_pair_swiglu_precondition_multiblock(
        input0, input1, **shared, packed_gate_up=True
    )
    assert torch.equal(actual.view(torch.int16), expected.view(torch.int16))
...
def test_qvq_cuda_folded_swiglu_precondition_is_exact_padded_and_graph_safe(
    ...
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = qvq_cuda_folded_swiglu_precondition_fp32(
            gate,
            up,
            gate_scale=gate_scale,
            up_scale=up_scale,
...
def test_qvq_cuda_qwen_ordered_composite_recovery_is_exact_and_graph_safe(
    ...
    graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):
        captured = qvq_cuda_qwen_composite_ordered_recovery_fp32_to_fp16(
            partials,
            base=base,
            post_scale=post_scale,
            bias=bias,
...
def test_qvq_cuda_w3_unified_factored_families_follows_fast_tf32_by_default(monkeypatch):
    monkeypatch.delenv("GPTQMODEL_QVQ_YAQA_W3_UNIFIED_FAMILIES", raising=False)
    monkeypatch.setenv("GPTQMODEL_QVQ_YAQA_FAST_TF32", "1")
    weight, input_hessian, output_hessian = _nontrivial_yaqa_fixture(20260925)
    banks = _canonical_qvq_v2b4_banks(
        device=weight.device,
        bits=3.0,
        codebook_version=PGC16_CODEBOOK_VERSION,
    ...
    family_batch.assert_called_once()
    assert family_batch.call_args.args[3].shape[0] == 3
    assert telemetry.finalize()["counters"]["yaqa_v2b2_wide_unified_family_batches"] == 1


def test_qvq_cuda_w3_unified_factored_families_explicit_opt_out(monkeypatch):
    monkeypatch.setenv("GPTQMODEL_QVQ_YAQA_FAST_TF32", "1")
    monkeypatch.setenv("GPTQMODEL_QVQ_YAQA_W3_UNIFIED_FAMILIES", "0")
    weight, input_hessian, output_hessian = _nontrivial_yaqa_fixture(20260926)
    banks = _canonical_qvq_v2b4_banks(
        device=weight.device,
        bits=3.0,
        codebook_version=PGC16_CODEBOOK_VERSION,
    ...
    assert telemetry.finalize()["counters"].get("yaqa_v2b2_wide_unified_family_batches", 0) == 0
