...
def _tiny_olmo3_model():
    config = Olmo3Config(
        ...
        rope_parameters={
            ...
            "full_attention": {"rope_type": "linear", "factor": 2.0, "rope_theta": 10_000.0},
        },
    )
...
def test_olmo3_quantization_groups_match_forward_boundaries():
    ...
    assert Olmo3QModel.get_module_tree_flags("self_attn.q_proj") == frozenset({"q"})
    assert Olmo3QModel.get_module_tree_flags("self_attn.k_proj") == frozenset({"k"})
    assert Olmo3QModel.get_module_tree_flags("self_attn.v_proj") == frozenset({"v"})
...
def test_olmo3_replay_refreshes_rope_for_each_attention_type():
    model = _tiny_olmo3_model()
    model_def = SimpleNamespace(model=model, rotary_embedding=Olmo3QModel.rotary_embedding)
    ...
    expected_full = model.model.rotary_emb(hidden_states, position_ids, "full_attention")
    assert all(torch.equal(actual, expected) for actual, expected in zip(full_kwargs["position_embeddings"], expected_full))
    assert not torch.equal(sliding_kwargs["position_embeddings"][0], full_kwargs["position_embeddings"][0])
