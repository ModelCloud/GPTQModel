...
def _compressed_entries(weight_key: str, tensor: torch.Tensor, group_size: int) -> dict[str, torch.Tensor]:
    q_key, scales_key, shape_key = compressed_weight_keys(weight_key)
    qweight, scales, shape = quantize_tensor_groupwise_int8(tensor, group_size=group_size)
    return {
        q_key: qweight,
        scales_key: scales,
        shape_key: shape,
...
def test_lora_loads_groupwise_int8_adapter_and_applies_dequantized_weights(tmp_path):
    torch.manual_seed(1)
    ...
    weights = {}
    weights.update(_compressed_entries(f"{save_key}.lora_A.weight", lora_a_saved, group_size))
    weights.update(_compressed_entries(f"{save_key}.lora_B.weight", lora_b_saved, group_size))
    save_file(weights, tmp_path / "adapter_model.safetensors", metadata={"format": "pt"})
    LoraConfig(
        r=rank,
        lora_alpha=rank,
        target_modules=["q_proj"],
        gptqmodel_lora_weight_format=LORA_INT8_FORMAT,
        gptqmodel_lora_weight_bits=8,
    ...
    expected_a = dequantize_tensor_groupwise_int8(
        qweight=q_a,
        scales=s_a,
        shape=sh_a,
    ...
    expected_b = dequantize_tensor_groupwise_int8(
        qweight=q_b,
        scales=s_b,
        shape=sh_b,
        group_size=group_size,
    ...
    assert adapter.lora_A_qweight.dtype == torch.int8
    torch.testing.assert_close(actual, expected, rtol=1e-5, atol=1e-5)


def test_lora_groupwise_int8_load_mode_materializes_bf16(tmp_path, monkeypatch):
    torch.manual_seed(2)
    rank = 4
    weight_key = "model.layers.0.mlp.up_proj"
    save_key = f"{HF_ADAPTER_WEIGHT_KEY_PREFIX}{weight_key}"
    weights = {}
    weights.update(_compressed_entries(f"{save_key}.lora_A.weight", torch.randn(rank, 16, dtype=torch.bfloat16), 8))
    weights.update(_compressed_entries(f"{save_key}.lora_B.weight", torch.randn(32, rank, dtype=torch.bfloat16), 8))
    save_file(weights, tmp_path / "adapter_model.safetensors", metadata={"format": "pt"})
    LoraConfig(
        r=rank,
        lora_alpha=rank,
    ...
    monkeypatch.setenv("GPTQMODEL_LORA_INT8_DEQUANT_MODE", "load")
