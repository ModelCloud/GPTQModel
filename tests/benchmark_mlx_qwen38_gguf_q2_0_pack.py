...
"""Benchmark main's FP32-temporary Q2_0 packer against direct MLX input."""
...
@lru_cache(maxsize=1)
def _main_q2_0_kernel():
    return mx.fast.metal_kernel(
        name="gptqmodel_gguf_q2_0_pack_main_benchmark",
        input_names=["weights"],
        output_names=["packed"],
        source="""
...
def _main_pack(weight):
    ...
    blocks = rows * columns // 64
    return _main_q2_0_kernel()(
        inputs=[weight.astype(mx.float32)],
        grid=(blocks, 1, 1),
        threadgroup=(min(blocks, 256), 1, 1),
        output_shapes=[(rows, columns // 64 * 18)],
...
def run(samples):
    ...
    for name, rows, columns in QWEN38_27B_PROJECTIONS:
        mx.random.seed(380002 + rows + columns)
        ...
        for label, dtype in (("FP16", mx.float16), ("BF16", mx.bfloat16)):
            ...
            def direct_fn(value=weight):
                return gguf_quantize_weight_mlx(value, "Q2_0")
