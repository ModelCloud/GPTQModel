...
def test_torch_ops_jit_extension_detects_python_markers_in_macro_definitions(tmp_path):
    source = tmp_path / "unit_test.cpp"
    source.write_text(
        "#define REGISTER_EXTENSION(NAME) \\\n"
        "  PyMODINIT_FUNC CONCAT(PyInit_, NAME)() { return PyModule_Create(nullptr); }\n",
        encoding="utf-8",
    )
    loader = _make_loader(tmp_path, sources=[str(source)])
...
def test_torch_ops_jit_extension_cpu_and_cuda_fingerprints_handle_wheel_switch(monkeypatch, tmp_path):
    ...
    assert cpu_loader._cache_fingerprint() != cpu_fingerprint
    assert cpu_loader._cache_fingerprint() != plain_fingerprint


def test_torch_ops_jit_extension_fingerprint_includes_stable_abi_target(monkeypatch, tmp_path):
    source = tmp_path / "unit_test.cpp"
    source.write_text(
    ...
    monkeypatch.setattr(cpp_module.torch, "__version__", "2.8.0+cpu")
    first_fingerprint = loader._cache_fingerprint()
    monkeypatch.setattr(cpp_module.torch, "__version__", "2.8.0+cu128")
    second_fingerprint = loader._cache_fingerprint()
...
def test_torch_ops_jit_extension_auto_detected_stable_sources_keep_torch_version(
...
def test_torch_ops_jit_extension_cpu_fingerprint_omits_torch_cuda(monkeypatch, tmp_path):
    ...
    loader = _make_loader(tmp_path, sources=[str(source)])
    first_fingerprint = loader._cache_fingerprint()
    monkeypatch.setattr(cpp_module.torch.version, "cuda", "12.8")
    second_fingerprint = loader._cache_fingerprint()
