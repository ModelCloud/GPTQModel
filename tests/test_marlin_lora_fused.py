...
def test_marlin_lora_addmm_tail_matches_dense(dtype):
    torch.manual_seed(0)
    ...
    actual = apply_marlin_fused_lora(adapter, x=x, out=base.clone())
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required")
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_marlin_lora_cuda_up_add_matches_dense(dtype, monkeypatch):
    monkeypatch.setenv("GPTQMODEL_MARLIN_LORA_CUDA_UP_ADD", "1")
    torch.manual_seed(1)
    device = torch.device("cuda:0")
...
def test_marlin_lora_cooperative_fused_tail_matches_fp32_reference(
    dtype, rows, rank, prepared, in_features, out_features, monkeypatch
):
    device = torch.device("cuda:0")
    if torch.cuda.get_device_capability(device) != (8, 0):
        pytest.skip("The cooperative LoRA kernel is currently enabled only for sm_80")
    ...
    if prepared:
        op = extension.op("marlin_lora", "lora_fused_add_prepared")
...
def test_marlin_lora_cooperative_uses_current_device_and_stream(device_index, monkeypatch):
...
def test_marlin_lora_fused_lora_falls_back_on_cpu():
    adapter = Lora(rank=4, lora_A=torch.randn(8, 4), lora_B=torch.randn(4, 16))
    x = torch.randn(2, 8)
    out = torch.randn(2, 16)

    assert apply_marlin_fused_lora(adapter, x=x, out=out) is None
