"""Reproduce the 128K-token calibration mix for Qwen/Qwen3-0.6B.

This script:
1. Downloads/loads `lemon07r/bartowski-imatrix-v5-semantic` and
   `neuralmagic/calibration` (LLM config).
2. Tokenizes the rows with the Qwen3-0.6B tokenizer and builds ~32K-token
   candidate shards.
...
Run with the free-threaded GPT-QModel venv and `PYTHON_GIL=0` for multi-threaded
search, e.g.:
...
def count_messages(tok, messages: list[dict[str, str]]) -> int:
    enc = tok.apply_chat_template(messages, tokenize=True, add_generation_prompt=False, return_dict=True)
...
def main() -> int:
    ...
    info = {
        ...
        "name": "128K-token calibration mix for Qwen3-0.6B (best-score floor)",
