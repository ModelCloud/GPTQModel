...
exec /root/venv-py3.14t/bin/python -m gpu_allocator.cli run -n 1 --style uuid -- \
  ...
  --output /root/qvq-results/w3-p32-gsq-ab-20260915/baseline \
  --report /root/qvq-results/w3-p32-gsq-ab-20260915/metadata/baseline.qvq_quantize_run.json \
  --quant-config /root/qvq-gsq-validation/artifacts/w3-p32-gsq-validation/baseline.quant-config.json \
