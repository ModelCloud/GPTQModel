...
exec /root/venv-py3.14t/bin/python -m gpu_allocator.cli run -n 1 --style uuid -- \
  ...
  --output /root/qvq-results/w3-p32-gsq-ab-20260915/evaluations/zml-sm90-fixed-canary/evaluation.json \
  --run-id w3-p32-gsq-ab-20260915 \
  --arm-id gsq-zml-sm90-fixed-fa2-b8-graph-canary \
  --rows 8 \
  --batch-size 8 \
  --context 8192 \
  --prefill-len 8192 \
  ...
  --log /root/qvq-results/w3-p32-gsq-ab-20260915/evaluations/zml-sm90-fixed-canary/evaluation.log
