...
exec /root/venv-py3.14t/bin/python -m gpu_allocator.cli run -n 1 --style uuid -- \
  /usr/bin/env PYTHONPATH=/root/qvq-gsq-validation \
  ...
  --output /root/qvq-results/w3-p32-gsq-corrected-20260916/model \
  --report /root/qvq-results/w3-p32-gsq-corrected-20260916/quantize_run.json \
  --quant-config /root/qvq-gsq-validation/artifacts/w3-p32-gsq-validation/gsq-corrected.quant-config.json \
