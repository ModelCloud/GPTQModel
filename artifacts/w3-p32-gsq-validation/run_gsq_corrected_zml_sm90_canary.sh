...
exec /root/venv-py3.14t/bin/python -m gpu_allocator.cli run -n 1 --style uuid -- \
  ...
  --output /root/qvq-results/w3-p32-gsq-corrected-20260916/evaluations/zml-sm90-canary/evaluation.json \
  --run-id w3-p32-gsq-corrected-20260916 \
  --arm-id gsq-corrected-zml-sm90-fa2-b8-graph-canary \
  --rows 8 \
  --batch-size 8 \
  --context 8192 \
  --prefill-len 8192 \
  ...
  --log /root/qvq-results/w3-p32-gsq-corrected-20260916/evaluations/zml-sm90-canary/evaluation.log
