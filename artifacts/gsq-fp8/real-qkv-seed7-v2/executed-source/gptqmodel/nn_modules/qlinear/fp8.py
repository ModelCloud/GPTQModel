    ...
    def dequantize_weight(
        ...
        *,
        ...
        if prefer_cpu_dequant:
            cpu_scale_inv = self.weight_scale_inv.to(device="cpu")
            # Generic checkpoint decoding guesses multiplicative semantics for
            # tables <= 1. This module explicitly owns inverse scales, including
            # that range, so bypass the heuristic when it would reinterpret them.
            small_inverse = bool(torch.max(torch.abs(cpu_scale_inv)) <= 1)
            weight = dequantize_fp8(
                self.weight.to(device="cpu"),
                scale_inv=None if small_inverse else cpu_scale_inv,
                axis=None if self.weight_scale_method == "block" else 0,
                target_dtype=target_dtype,
            )
            if small_inverse:
                weight = weight / self._expanded_scale_inv(target_device=torch.device("cpu"),
                                                           target_dtype=target_dtype)
