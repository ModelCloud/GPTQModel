...
def relaxed_scalar_weights(logits, scales, candidates, group_index, *, uniform, temperature, multiplier):
    ...
    probability = ((logits * multiplier + noise) / temperature).softmax(0)
    return (probability*candidates).sum(0) * scales[:, group_index]
...
class GSQScalarTrainingModule(torch.nn.Module):
    ...
    def __init__(self, weight, scales, group_size, *, bits, noise, std=.01, strength=6.):
        super().__init__()
        prepared = scalar_training_candidates(weight, scales, group_size, bits=bits,
                                               noise=noise, std=std, strength=strength)
        self.logits = torch.nn.Parameter(prepared['logits'])
    ...
    def forward(self, *, uniform, temperature, multiplier):
        ...
        return relaxed_scalar_weights(masked, self.scales, self.candidates, self.group_index,
...
def fit_reconstruction_stage(quantizers, batches, objective, *, epochs, seed=7,
                             assignment_lr=2e-4, scale_lr=1e-4, weight_decay=1., betas=(.9, .99),
...
def fit_llama_stages(layer, initializers, batches, *, bits, group_size, epochs, seed=7, **training):
    ...
    for name in names[:2]:
        ...
        normalized = []
        with torch.no_grad():
            for hidden, _ in batches:
                inputs = fitted.input_layernorm(hidden)
                normalized.append([(inputs, inputs.numel()//inputs.shape[-1]*projection.out_features)])

        def linear_objective(inputs, weights, projection=projection):
            return reconstruction_stage_loss(projection, (inputs,), {}, student_weights=weights)
        result = fit_reconstruction_stage({'weight': quant}, normalized, linear_objective,
                                          epochs=epochs, seed=seed, **training)
        with torch.no_grad():
