"""Paired document bootstrap for a completed quantization propagation diagnostic."""
...
def analyze(path, arms=('gsq_weight', 'gsq_calibrated'), baseline_arm='baseline'):
    report = json.loads(path.read_text())
    if report['state'] != 'complete':
        raise ValueError('Requires all arms completed')
    baseline = report['arms'][baseline_arm]['rows']
    ...
    result = {'method': 'paired document bootstrap; token-weighted means', 'seed': 7,
              'draws': 10000, 'baseline': baseline_arm, 'arms': {}}
    for arm in arms:
        rows = report['arms'][arm]['rows']
        if len(rows) != len(baseline) or any(
                a['tokens'] != b['tokens'] or a['teacher_sha256'] != b['teacher_sha256']
...
if __name__ == '__main__':
    ...
    args.output.write_text(json.dumps(analyze(args.report, arms=args.arms, baseline_arm=args.baseline), indent=2)+'\n')
