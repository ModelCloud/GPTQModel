# Deterministic configured W2 staged block

State: complete. Real Llama 3.2 1B Instruct, 16 calibration / 32 held-out documents, group128, FP32, seed7. Same-source complete repeat matches initializers, all final weights/scales, every stage loss/schedule and held-out metrics exactly. Comparison: `../llama-block0-w2-seed7-config-deterministic-repeat-v6/comparison.json`. This proves repeatability for this runtime/configuration, not full-paper procedure parity or new final-logit recovery.

```sh
PYTHONPATH=. CUDA_DEVICE_ORDER=PCI_BUS_ID CUBLAS_WORKSPACE_CONFIG=:4096:8 OMP_NUM_THREADS=4 MAX_JOBS=4 /root/venv-py3.14t/bin/python -m gpu_allocator.cli run -n 1 --style uuid -- /root/venv-py3.14t/bin/python scripts/validate_gsq_staged_llama.py --inputs artifacts/gsq-scalar/gptq-w4-seed7-v2 --output artifacts/gsq-staged/llama-block0-w2-seed7-config-deterministic-repeat-v6 --bits 2 --epochs 10 --qk-steps 2000 --damp-percent .01 --deterministic
```
...
```json
{
  "metrics": {
    ...
    "staged": 0.000330968996554516
  },
  "relative_change_percent": -46.4733902354275,
  "log": "artifacts/gsq-staged/logs/llama-block0-w2-seed7-config-deterministic-repeat-v6.log",
  "log_sha256": "817484706bd074fdb743b18219c255678574f290f84217cb25ef832086c3a791"
}
```

Configuration, source hashes, model paths and hardware/runtime:
...
```json
{
  ...
  "source_hashes": {
    "/root/polly-work/qvq-gsq/scripts/validate_gsq_staged_llama.py": "8e75339acd635098c7ebe51636f1e43020cced255379477a63f61f2af1dff6bb",
  ...
  "started_utc": "2026-09-09T09:59:55.155661+00:00",
  "argv": [
    "scripts/validate_gsq_staged_llama.py",
    "--inputs",
    "artifacts/gsq-scalar/gptq-w4-seed7-v2",
    "--output",
    "artifacts/gsq-staged/llama-block0-w2-seed7-config-deterministic-repeat-v6",
    "--bits",
    "2",
    "--epochs",
    "10",
  ...
  "run_id": "llama-block0-w2-seed7-config-deterministic-repeat-v6",
  ...
  "finished_utc": "2026-09-09T10:00:37.707118+00:00"
}
```
