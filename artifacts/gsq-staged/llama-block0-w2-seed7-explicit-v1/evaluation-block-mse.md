# W2 staged GSQ local reconstruction

Completed execution. Selected-layer artifact, not portable full-model export. ZML N/A.

Artifact: /root/polly-work/qvq-gsq/artifacts/gsq-staged/llama-block0-w2-seed7-explicit-v1

CLI from /root/polly-work/qvq-gsq: `PYTHONPATH=. CUDA_DEVICE_ORDER=PCI_BUS_ID OMP_NUM_THREADS=4 MAX_JOBS=4 /root/venv-py3.14t/bin/python -m gpu_allocator.cli run -n 1 --style uuid -- /root/venv-py3.14t/bin/python -m scripts.validate_gsq_staged_llama --inputs artifacts/gsq-scalar/gptq-w4-seed7-v2 --output artifacts/gsq-staged/llama-block0-w2-seed7-explicit-v1 --bits 2 --epochs 10 --qk-steps 2000 --damp-percent .01`

Unweighted FP32 block0 diagnostic; not F6/N production comparison or full paper reproduction. Bootstrap resamples paired documents, weights by reconstructed output elements. Local improvement requires final-logit and packed-export validation.

```json
{
  "baseline_mse": 0.000618326096142135,
  "staged_mse": 0.00034859923737329105,
  "relative_change_percent": -43.62210497854221,
  "paired_mse_delta_ci95": [
    -0.00030742863429031574,
    -0.0002416895911796883
  ],
  "bootstrap_draws": 10000,
  "bootstrap_seed": 7,
  "heldout_documents": 32,
  "elements": 13039616,
  "log_path": "/root/polly-work/qvq-gsq/artifacts/gsq-staged/logs/llama-block0-w2-seed7-explicit-v1.log",
  "log_sha256": "6d932318363c4c4922b3249d982c4d14230e1c07982f5ddbc850d833b34af056"
}
```

```json
{
  "state": "complete",
  "commit": "bebfd81fcf384363e4b15b72e35656c9f35af083",
  "source_hashes": {
    "/root/polly-work/qvq-gsq/scripts/validate_gsq_staged_llama.py": "9ac5a2122601b1d74a6e9615dd2daf26ce190b3fa6f8a56f598afc46dabe85a2",
    "/root/polly-work/qvq-gsq/gptqmodel/quantization/gsq_training.py": "f4492e5b0039e017d5ea60a549f15887668e582bd7c9030e830810c35920e48d",
    "/root/polly-work/qvq-gsq/artifacts/gsq-scalar/gptq-w4-seed7-v2/inputs.json": "1fad35585b53bcf26beb66acb0c1fa5b5062404d889cbe02cd87c43cddedaff1",
    "/root/polly-work/qvq-gsq/artifacts/gsq-scalar/gptq-w4-seed7-v2/provenance.json": "747b19ae35e95e21f06c02f34a54c55d28cecea86aaecdbe7e30c06463d458b8",
    "/monster/data/model/Llama-3.2-1B-Instruct/model.safetensors": "1ff795ff6a07e6a68085d206fb84417da2f083f68391c2843cd2b8ac6df8538f",
    "/monster/data/model/Llama-3.2-1B-Instruct/config.json": "2febf68cea25bf4611be02b7536f2488a5ba523bb1134986e3610152abe74fdb"
  },
  "inventory": "0, 00000000:DE:00.0, GPU-737e2423-874a-23a4-1126-dfbe3e77c294, NVIDIA PG506-230, 0, 0",
  "torch": "2.15.0.dev20260817+cu130",
  "cuda": "13.0",
  "seed": 7,
  "bits": 2,
  "epochs": 10,
  "qk_steps": 2000,
  "group_size": 128,
  "attention": "eager",
  "cache": false,
  "graphs": false,
  "precision": "float32",
  "source_model": "/monster/data/model/Llama-3.2-1B-Instruct",
  "scope": "block0, local reconstruction; no paper-reproduction claim",
  "training": {
    "assignment_lr": 0.0001,
    "scale_lr": 5e-05,
    "betas": [
      0.9,
      0.95
    ],
    "weight_decay": 1.0,
    "temperature": [
      2.0,
      0.05
    ],
    "multiplier": [
      100.0,
      500.0
    ],
    "warmup_steps": 0,
    "min_lr": 0.1,
    "decay": "cosine"
  },
  "qk_learning_rate_decay": "constant",
  "qk_damp_percent": 0.01,
  "initializer_damp_percent": 0.01,
  "started_utc": "2026-09-09T09:38:00.234809+00:00",
  "argv": [
    "/root/polly-work/qvq-gsq/scripts/validate_gsq_staged_llama.py",
    "--inputs",
    "artifacts/gsq-scalar/gptq-w4-seed7-v2",
    "--output",
    "artifacts/gsq-staged/llama-block0-w2-seed7-explicit-v1",
    "--bits",
    "2",
    "--epochs",
    "10",
    "--qk-steps",
    "2000",
    "--damp-percent",
    ".01"
  ],
  "run_id": "llama-block0-w2-seed7-explicit-v1",
  "weighting": "unweighted documents; not YAQA 1.25/N-mode reproduction",
  "gpu_properties": "_CudaDeviceProperties(name='NVIDIA PG506-230', major=8, minor=0, total_memory=97457MB, multi_processor_count=124, uuid=737e2423-874a-23a4-1126-dfbe3e77c294, pci_bus_id=222, pci_device_id=0, pci_domain_id=0, L2_cache_size=48MB)",
  "initializer": {
    "self_attn.q_proj": {
      "loss": 0.00401970509163796,
      "damp": 0.009999999776482582,
      "samples": 3767
    },
    "self_attn.k_proj": {
      "loss": 0.0018991010082093987,
      "damp": 0.009999999776482582,
      "samples": 3767
    },
    "self_attn.v_proj": {
      "loss": 3.86399491130119e-05,
      "damp": 0.009999999776482582,
      "samples": 3767
    },
    "self_attn.o_proj": {
      "loss": 3.6901146379174117e-06,
      "damp": 0.009999999776482582,
      "samples": 3767
    },
    "mlp.gate_proj": {
      "loss": 0.0027891715515314945,
      "damp": 0.009999999776482582,
      "samples": 3767
    },
    "mlp.up_proj": {
      "loss": 0.002140536307022747,
      "damp": 0.009999999776482582,
      "samples": 3767
    },
    "mlp.down_proj": {
      "loss": 1.383828355742602e-05,
      "damp": 0.009999999776482582,
      "samples": 3767
    }
  },
  "heldout": [
    {
      "elements": 374784,
      "baseline_sse": 201.6371654609233,
      "staged_sse": 103.69058720013984
    },
    {
      "elements": 524288,
      "baseline_sse": 268.9656104950283,
      "staged_sse": 165.95044334240035
    },
    {
      "elements": 413696,
      "baseline_sse": 227.72612558742986,
      "staged_sse": 129.2296060741575
    },
    {
      "elements": 245760,
      "baseline_sse": 131.6985245766103,
      "staged_sse": 55.3468900035294
    },
    {
      "elements": 524288,
      "baseline_sse": 242.06756688642616,
      "staged_sse": 136.59642069152204
    },
    {
      "elements": 514048,
      "baseline_sse": 220.2638772081244,
      "staged_sse": 133.23571366287706
    },
    {
      "elements": 524288,
      "baseline_sse": 455.99980527269423,
      "staged_sse": 208.37005144468134
    },
    {
      "elements": 393216,
      "baseline_sse": 288.4139035893474,
      "staged_sse": 146.1411156386856
    },
    {
      "elements": 350208,
      "baseline_sse": 196.8114816080141,
      "staged_sse": 106.94713547197907
    },
    {
      "elements": 524288,
      "baseline_sse": 260.40308395409835,
      "staged_sse": 153.63996290796098
    },
    {
      "elements": 524288,
      "baseline_sse": 297.45335008164886,
      "staged_sse": 183.87488193652547
    },
    {
      "elements": 524288,
      "baseline_sse": 316.35818175730174,
      "staged_sse": 192.8172463140486
    },
    {
      "elements": 303104,
      "baseline_sse": 203.7402491956413,
      "staged_sse": 105.58603358655908
    },
    {
      "elements": 466944,
      "baseline_sse": 260.9742349871046,
      "staged_sse": 159.73589080188412
    },
    {
      "elements": 524288,
      "baseline_sse": 371.5390567478093,
      "staged_sse": 199.60499439093044
    },
    {
      "elements": 264192,
      "baseline_sse": 260.63275234655543,
      "staged_sse": 107.74908504909043
    },
    {
      "elements": 524288,
      "baseline_sse": 293.7800773574379,
      "staged_sse": 193.99805507090224
    },
    {
      "elements": 524288,
      "baseline_sse": 355.4588717678584,
      "staged_sse": 223.88498583624906
    },
    {
      "elements": 329728,
      "baseline_sse": 156.16950701264187,
      "staged_sse": 70.58314054927233
    },
    {
      "elements": 196608,
      "baseline_sse": 117.20278782792509,
      "staged_sse": 34.88380244664285
    },
    {
      "elements": 237568,
      "baseline_sse": 226.29763189677567,
      "staged_sse": 132.75826117492105
    },
    {
      "elements": 176128,
      "baseline_sse": 129.53716168830977,
      "staged_sse": 50.3254082074226
    },
    {
      "elements": 165888,
      "baseline_sse": 157.7736371044227,
      "staged_sse": 75.0537409509229
    },
    {
      "elements": 116736,
      "baseline_sse": 105.91642189892536,
      "staged_sse": 31.924231172376306
    },
    {
      "elements": 102400,
      "baseline_sse": 121.64191519832195,
      "staged_sse": 32.01913638737322
    },
    {
      "elements": 524288,
      "baseline_sse": 302.3763424370926,
      "staged_sse": 188.32607630469573
    },
    {
      "elements": 524288,
      "baseline_sse": 329.1768007961418,
      "staged_sse": 192.96659493519996
    },
    {
      "elements": 524288,
      "baseline_sse": 283.2336425345176,
      "staged_sse": 169.94428634990558
    },
    {
      "elements": 524288,
      "baseline_sse": 308.57872302501283,
      "staged_sse": 201.12533701374983
    },
    {
      "elements": 524288,
      "baseline_sse": 401.41850279470316,
      "staged_sse": 283.23165201980635
    },
    {
      "elements": 524288,
      "baseline_sse": 255.73066680351832,
      "staged_sse": 162.59954009937587
    },
    {
      "elements": 524288,
      "baseline_sse": 313.7571965741589,
      "staged_sse": 213.45988620477738
    }
  ],
  "payload_sha256": "fad22b272bc47f6341dc373a4e289520caabe0255dd406c5301336ace922ea6c",
  "finished_utc": "2026-09-09T09:38:42.449044+00:00"
}
```
