...
torch::Tensor amplin_gptq_w4_group128_gemm_hmma_m32_n128_pipeline4_dispatch(
    torch::Tensor input,
    torch::Tensor packed_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_gptq_w4_group128_gemm_hmma_m32_n128_pipeline4_cuda(
      input,
      packed_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_tile2_shared_a_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_tile2_shared_a_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_tile2_splitk2_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_tile2_splitk2_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_tile1_splitk4_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_tile1_splitk4_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_tile1_splitk8_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_tile1_splitk8_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_tile2_splitk4_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_tile2_splitk4_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m16_n64_splitk4_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m16_n64_splitk4_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m16_n64_splitk8_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m16_n64_splitk8_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m16_n64_splitk12_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m16_n64_splitk12_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m16_n64_splitk16_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m16_n64_splitk16_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m16_n64_splitk20_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m16_n64_splitk20_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_splitk12_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_splitk12_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_splitk16_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_splitk16_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
torch::Tensor amplin_mma_lane_m32_n64_splitk20_pipe2_interleaved_dispatch(
    torch::Tensor input,
    torch::Tensor packed_lane_qweight,
    torch::Tensor packed_scales,
    int64_t logical_n) {
  return amplin_mma_lane_m32_n64_splitk20_pipe2_interleaved_cuda(
      input,
      packed_lane_qweight,
      packed_scales,
      logical_n);
...
TORCH_LIBRARY(gptqmodel_amplin, m) {
  ...
  m.def("gemm_hmma_m32_n128_pipeline4(Tensor input, Tensor packed_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  ...
  m.def(
      "mma_lane_m32_n64_tile2_splitk2(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m32_n64_tile1_splitk4(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m32_n64_tile1_splitk8(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m32_n64_tile2_splitk4(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
  ...
  m.def(
      "mma_lane_m32_n64_splitk12_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m32_n64_splitk16_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m32_n64_splitk20_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
  ...
  m.def(
      "mma_lane_m16_n64_splitk4_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m16_n64_splitk8_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m16_n64_splitk12_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m16_n64_splitk16_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
      "mma_lane_m16_n64_splitk20_pipe2_interleaved(Tensor input, Tensor packed_lane_qweight, Tensor packed_scales, int logical_n) -> Tensor");
  m.def(
...
TORCH_LIBRARY_IMPL(gptqmodel_amplin, CUDA, m) {
  ...
  m.impl(
      ...
      &amplin_gptq_w4_group128_gemm_hmma_m32_n128_pipeline4_dispatch);
  ...
  m.impl(
      "mma_lane_m32_n64_tile2_splitk2", &amplin_mma_lane_m32_n64_tile2_splitk2_dispatch);
  m.impl(
      "mma_lane_m32_n64_tile1_splitk4", &amplin_mma_lane_m32_n64_tile1_splitk4_dispatch);
  m.impl(
      "mma_lane_m32_n64_tile1_splitk8", &amplin_mma_lane_m32_n64_tile1_splitk8_dispatch);
  m.impl(
      "mma_lane_m32_n64_tile2_splitk4", &amplin_mma_lane_m32_n64_tile2_splitk4_dispatch);
  m.impl(
  ...
  m.impl(
      ...
      &amplin_mma_lane_m32_n64_splitk12_pipe2_interleaved_dispatch);
  m.impl(
      "mma_lane_m32_n64_splitk16_pipe2_interleaved",
      &amplin_mma_lane_m32_n64_splitk16_pipe2_interleaved_dispatch);
  m.impl(
      "mma_lane_m32_n64_splitk20_pipe2_interleaved",
      &amplin_mma_lane_m32_n64_splitk20_pipe2_interleaved_dispatch);
  ...
  m.impl(
      ...
      &amplin_mma_lane_m16_n64_splitk4_pipe2_interleaved_dispatch);
  m.impl(
      "mma_lane_m16_n64_splitk8_pipe2_interleaved",
      &amplin_mma_lane_m16_n64_splitk8_pipe2_interleaved_dispatch);
  m.impl(
      "mma_lane_m16_n64_splitk12_pipe2_interleaved",
      &amplin_mma_lane_m16_n64_splitk12_pipe2_interleaved_dispatch);
  m.impl(
      "mma_lane_m16_n64_splitk16_pipe2_interleaved",
      &amplin_mma_lane_m16_n64_splitk16_pipe2_interleaved_dispatch);
  m.impl(
      "mma_lane_m16_n64_splitk20_pipe2_interleaved",
      &amplin_mma_lane_m16_n64_splitk20_pipe2_interleaved_dispatch);
  m.impl(
