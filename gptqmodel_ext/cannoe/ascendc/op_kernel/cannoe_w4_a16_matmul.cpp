        ...
        for (uint32_t group = first_group; group <= last_group; ++group) {
            ...
            for (uint32_t packed_col = packed_begin; packed_col < packed_end;) {
                if (zero_offsets != 0 && packed_col + 3 < packed_end) {
                    ...
                    for (uint32_t k = group_k_begin; k < group_k_end; ++k) {
                        const uint32_t tile_k = k - k_begin;
                        const uint32_t packed_row = k * packed_stride + packed_col;
                        const uint32_t word0 = static_cast<uint32_t>(packed_weight_gm_.GetValue(packed_row));
                        const uint32_t word1 = static_cast<uint32_t>(packed_weight_gm_.GetValue(packed_row + 1));
                        const uint32_t word2 = static_cast<uint32_t>(packed_weight_gm_.GetValue(packed_row + 2));
                        const uint32_t word3 = static_cast<uint32_t>(packed_weight_gm_.GetValue(packed_row + 3));
                        FillDirectBTileWordValuesNoOffset(
                            b_tile,
                            tile_k * base_n + tile_n0,
                            word0,
