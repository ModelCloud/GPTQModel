...
Cannoe fused W4A16 Ascend C operator. Cannoe-specific torch-op names and
environment variables use the `cannoe` spelling only.
...
- `torch.ops.npu.gptqmodel_cannoe_w4_a16_matmul`
...
`op_ir/cannoe_w4a16_matmul.json` can be passed to `msopgen` as a starting
point for an Ascend C project:

```bash
msopgen gen \
  -i gptqmodel_ext/cannoe/op_ir/cannoe_w4a16_matmul.json \
  -f pytorch \
  -c ai_core-ascend910b \
  -lan cpp \
  -out /tmp/cannoe_w4a16_op
```

After the generated project registers a torch-visible operator, use
`GPTQMODEL_CANNOE_FUSED_REQUIRE=1` while profiling so a missing op cannot
...
`torch.ops.gptqmodel_cannoe.w4a16_matmul` and calls the raw
...
python scripts/profile_cannoe_npu.py --mode cannoe --iters 3 --warmup 1
```
...
`GPTQMODEL_CANNOE_INNER_PRECISE=auto|0|1` controls the CANN
...
`hw/cannoe.md` record the measured liked and avoided shapes.

The V3 bridge also caches repeatable ACL executors and reusable workspace by
default:

```bash
GPTQMODEL_CANNOE_V3_EXECUTOR_CACHE=1    # default
GPTQMODEL_CANNOE_V3_WORKSPACE_CACHE=1   # default
```
...
`ascendc/` contains the repo-owned custom-op overlay for the real Cannoe
...
```bash
/tmp/cannoe_w4a16_op/build_out/custom_opp_ubuntu_aarch64.run \
  --quiet \
  --install-path=/tmp/cannoe_opp_install
source /tmp/cannoe_opp_install/vendors/customize/bin/set_env.bash
```

Enable the managed bridge with:

```bash
GPTQMODEL_CANNOE_ASCENDC=1 \
GPTQMODEL_CANNOE_FUSED_REQUIRE=1 \
GPTQMODEL_CANNOE_FUSED_OP=gptqmodel_cannoe.cannoe_w4_a16_matmul \
python scripts/profile_cannoe_npu.py --mode cannoe --iters 3 --warmup 1
```
...
`GPTQMODEL_CANNOE_STAGED_DEQUANT=1` enables bounded FP16 tile staging, and
`GPTQMODEL_CANNOE_CUBE_CONSUMER=1` additionally reserves the leading CANN
