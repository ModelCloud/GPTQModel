...
thread_config_t mentaray_lookup_thread_config(int thread_k, int thread_n) {
  thread_config_t const* config_sets[] = {
      small_batch_thread_configs,
      large_batch_thread_configs,
  };
  int config_set_sizes[] = {
  ...
  for (int set_idx = 0; set_idx < 2; ++set_idx) {
    thread_config_t const* thread_configs = config_sets[set_idx];
    int thread_configs_size = config_set_sizes[set_idx];
    for (int i = 0; i < thread_configs_size; ++i) {
      if (thread_configs[i].thread_k == thread_k &&
          thread_configs[i].thread_n == thread_n) {
        return thread_configs[i];
      }
    }
  }

  return thread_config_t{thread_k, thread_n, default_threads};
}

int mentaray_env_override(const char* name, int default_value = -1) {
...
bool mentaray_prefers_full_sm80(int sms) {
  return sms >= 120;
}

int mentaray_blocks_per_sm_limit(int sms) {
  (void)sms;
...
exec_config_t determine_exec_config(const vllm::ScalarType& q_type, int prob_m,
  ...
  int blocks_per_sm_limit = mentaray_blocks_per_sm_limit(sms);
  // A100 124-SM sweeps showed 64x128 stays ahead through the mid-batch range,
  // while 64x256 only starts to win consistently once M reaches ~128.
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* b_bias,
  ...
  while (rest_m) {
    ...
    if (thread_k == -1) {
      thread_k = mentaray_env_override("GPTQMODEL_MENTARAY_THREAD_K");
    }
    if (thread_n == -1) {
...
torch::Tensor MARLIN_GEMM_EXPORT_NAME(
  ...
  if (a.scalar_type() == at::ScalarType::BFloat16) {
    ...
    mentaray::marlin_mm<nv_bfloat16>(
