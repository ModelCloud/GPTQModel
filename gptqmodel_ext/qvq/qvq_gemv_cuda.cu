      ...
      if (u < tiles_here) {
        for (int sub = 0; sub < kOutputTilesPerBlock; ++sub) {
          ...
          if constexpr (TransitionBits != 0) {
            ...
            if (k_local == 0) {
              const uint32_t bank = (static_cast<uint32_t>(packed_bank_ids[u][sub]) >> ring & 1u) *
                  static_cast<uint32_t>(bank_alt_id);
            ...
            weight = qvq_decode_local_ring_weight_fast<TransitionBits>(
                packed_words[u][sub], cached_levels, ring, k_local, bank_mask);
          } else {
...
// The ROWS=8 specialization uses two independent 256-thread N8 subgroups in
// one 512-thread block. Each subgroup retains the single-tile register
// footprint of the normal kernel, while the block shares its input staging and
// halves the N-tile block count to match the legacy N16 geometry.
template <typename Scalar, typename OutputScalar, int TransitionBits, bool SplitK, bool VectorStaging>
__global__ __launch_bounds__(2 * kThreads) void qvq_gemv_local_ring_fused2_kernel(
    const Scalar* __restrict__ input,
    const int32_t* __restrict__ trellis,
    const uint8_t* __restrict__ bank_ids,
    const half* __restrict__ levels,
  ...
  __shared__ __align__(16) uint32_t packed_words[kBatchTiles][kOutputTilesPerBlock][kMaxWords];
  __shared__ uint8_t packed_bank_ids[kBatchTiles][kOutputTilesPerBlock];
  __shared__ __align__(16) Scalar input_tile[kBatchTiles][kLocalRingTileRows * 8];
  ...
  const int n_tile_base = static_cast<int>(blockIdx.x) * kOutputTilesPerBlock;
  const int m0 = static_cast<int>(blockIdx.y) * 8;
  const int block_rows = min(8, size_m - m0);
  const int n_tiles = size_n / kLocalRingTileColumns;
  const int k_tiles = size_k / kLocalRingTileRows;
...
#pragma unroll
  ...
  auto stage_batch = [&](int kb, int tiles_here) {
    if constexpr (VectorStaging && !kQvqDebugDisableVecStaging) {
      constexpr int words_per_vec = kWordsPerTile / 4;
      const int vec_total = tiles_here * kOutputTilesPerBlock * words_per_vec;
      uint4* words4 = reinterpret_cast<uint4*>(packed_words[0][0]);
      const uint4* trellis4 = reinterpret_cast<const uint4*>(trellis);
      for (int index = thread; index < vec_total; index += kFusedThreads) {
        const int tile_slot = index / words_per_vec;
        const int w4 = index - tile_slot * words_per_vec;
        const int u = tile_slot / kOutputTilesPerBlock;
      ...
      for (int index = thread; index < tiles_here * kOutputTilesPerBlock * kWordsPerTile; index += kFusedThreads) {
        const int tile_slot = index / kWordsPerTile;
        const int w = index - tile_slot * kWordsPerTile;
        const int u = tile_slot / kOutputTilesPerBlock;
        const int tile_sub = tile_slot % kOutputTilesPerBlock;
        const int n_tile = n_tile_base + tile_sub;
        const int tile_index = (kb + u) * n_tiles + n_tile;
        packed_words[u][tile_sub][w] =
            n_tile < n_tiles ? static_cast<uint32_t>(trellis[static_cast<int64_t>(tile_index) * kWordsPerTile + w])
                             : 0u;
      }
    }
    if (thread < tiles_here * kOutputTilesPerBlock) {
    ...
    // Only the first subgroup loads the shared input tile. Both subgroups
    // consume the same activation stripe for their two adjacent N8 outputs.
    if (sub == 0) {
      if constexpr (VectorStaging && !kQvqDebugDisableVecStaging) {
        const int vec_total = tiles_here * 8 * kInputVecsPerRow;
        uint4* input4 = reinterpret_cast<uint4*>(input_tile[0]);
        const uint4* input4_base = reinterpret_cast<const uint4*>(input);
        for (int index = tile_thread; index < vec_total; index += kThreads) {
          const int u = index / (8 * kInputVecsPerRow);
          const int cell = index - u * (8 * kInputVecsPerRow);
          const int row = cell / kInputVecsPerRow;
          const int vec = cell - row * kInputVecsPerRow;
          if (row < block_rows) {
            input4[index] = input4_base[
        ...
        for (int index = tile_thread; index < tiles_here * 8 * kLocalRingTileRows; index += kThreads) {
          const int u = index / (8 * kLocalRingTileRows);
          const int cell = index - u * (8 * kLocalRingTileRows);
          const int row = cell / kLocalRingTileRows;
          const int k_local_tile = cell - row * kLocalRingTileRows;
          input_tile[u][cell] = row < block_rows
              ? input[static_cast<int64_t>(m0 + row) * size_k + (kb + u) * kLocalRingTileRows + k_local_tile]
...
#pragma unroll
    for (int u = 0; u < kBatchTiles; ++u) {
      if (u < tiles_here && n_tile_base + sub < n_tiles) {
        ...
        bank_mask = __shfl_sync(0xffffffffu, bank_mask, 0);
        const float weight = qvq_decode_local_ring_weight_fast<TransitionBits>(
            packed_words[u][sub], cached_levels, ring, k_local, bank_mask);
#pragma unroll
        for (int r = 0; r < 8; ++r) {
          accumulator[r] = fmaf(
...
template <typename Scalar, typename OutputScalar, bool SplitK, bool VectorStaging>
void launch_qvq_local_ring_fused2_gemv(
    const at::Tensor& input,
    const at::Tensor& trellis,
    const at::Tensor& bank_ids,
    const at::Tensor& levels,
  ...
  const dim3 grid(
      ...
      static_cast<unsigned int>((input.size(0) + 7) / 8),
      static_cast<unsigned int>(SplitK ? split_count : 1));
  const Scalar* input_ptr = reinterpret_cast<const Scalar*>(input.const_data_ptr());
  const half* levels_ptr = reinterpret_cast<const half*>(levels.const_data_ptr());
  OutputScalar* output_ptr = reinterpret_cast<OutputScalar*>(output.mutable_data_ptr());
...
#define QVQ_LR_FUSED2_LAUNCH(TB)                                                                                   \
  qvq_gemv_local_ring_fused2_kernel<Scalar, OutputScalar, TB, SplitK, VectorStaging>                                \
  ...
  switch (transition_bits) {
    case 2:
      QVQ_LR_FUSED2_LAUNCH(2);
      break;
    case 3:
      QVQ_LR_FUSED2_LAUNCH(3);
      break;
    case 4:
      QVQ_LR_FUSED2_LAUNCH(4);
      break;
    case 5:
      QVQ_LR_FUSED2_LAUNCH(5);
      break;
    case 6:
      QVQ_LR_FUSED2_LAUNCH(6);
      break;
    case 7:
      QVQ_LR_FUSED2_LAUNCH(7);
      break;
    default:
      TORCH_CHECK(false, "LR32 transition_bits must be in [2, 7]");
  }
#undef QVQ_LR_FUSED2_LAUNCH
...
void launch_qvq_local_ring_gemv(
  ...
  if (qvq_rows_for_m(static_cast<int>(input.size(0))) == 8 && vector_staging) {
    launch_qvq_local_ring_fused2_gemv<Scalar, OutputScalar, SplitK, true>(
        input,
        trellis,
        bank_ids,
        levels,
  ...
  if (qvq_rows_for_m(static_cast<int>(input.size(0))) == 8) {
    launch_qvq_local_ring_fused2_gemv<Scalar, OutputScalar, SplitK, false>(
        input,
        trellis,
        bank_ids,
        levels,
