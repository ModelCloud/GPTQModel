...
torch::Tensor gptq_marlin_gemm_prepared_bf16(
...
torch::Tensor gptq_marlin_gemm_eora_bf16_dispatch(
    torch::Tensor a, std::optional<torch::Tensor> c_or_none,
    torch::Tensor b_q_weight, std::optional<torch::Tensor> const& b_bias_or_none,
    torch::Tensor b_scales,
    std::optional<torch::Tensor> const& global_scale_or_none,
    ...
    torch::Tensor up_weight, torch::Tensor eora_workspace,
    int64_t b_q_type_id, int64_t size_m, int64_t size_n, int64_t size_k,
    bool is_k_full, bool use_atomic_add, bool use_fp32_reduce,
    bool is_zp_float, bool use_packed_prefill, int64_t packed_prefill_config) {
  torch::Tensor out = gptq_marlin_gemm_bf16(
  ...
  return eora_marlin_lora_fused_add_prepared_cuda(
      a, down_weight, up_weight, out, eora_workspace);
}

torch::Tensor gptq_marlin_gemm_eora_prepared_bf16_dispatch(
    torch::Tensor a, torch::Tensor b_q_weight, torch::Tensor b_scales,
    torch::Tensor marlin_workspace, torch::Tensor down_weight,
    torch::Tensor up_weight, bool use_packed_prefill,
    int64_t packed_prefill_config) {
  return gptq_marlin_gemm_prepared_bf16(
...
TORCH_LIBRARY(gptqmodel_marlin_bf16, m) {
  ...
  m.def(
      "gptq_marlin_gemm_eora_bf16(Tensor a, Tensor? c, Tensor b_q_weight, Tensor? b_bias, "
      "Tensor b_scales, Tensor? global_scale, Tensor? b_zeros, Tensor? g_idx, Tensor? perm, "
      "Tensor marlin_workspace, Tensor down_weight, Tensor up_weight, Tensor(b!) eora_workspace, "
  ...
  m.def(
      "gptq_marlin_gemm_eora_prepared_bf16(Tensor a, Tensor b_q_weight, Tensor b_scales, "
...
TORCH_LIBRARY_IMPL(gptqmodel_marlin_bf16, CUDA, m) {
  ...
  m.impl("gptq_marlin_gemm_eora_bf16",
         &gptq_marlin_gemm_eora_bf16_dispatch);
  m.impl("gptq_marlin_gemm_eora_prepared_bf16",
         &gptq_marlin_gemm_eora_prepared_bf16_dispatch);
