...
torch::Tensor gptq_marlin_gemm_eora_fp16_dispatch(
    torch::Tensor a, std::optional<torch::Tensor> c_or_none,
    torch::Tensor b_q_weight, std::optional<torch::Tensor> const& b_bias_or_none,
    torch::Tensor b_scales,
    std::optional<torch::Tensor> const& global_scale_or_none,
...
torch::Tensor gptq_marlin_gemm_eora_prepared_fp16_dispatch(
    torch::Tensor a, torch::Tensor b_q_weight, torch::Tensor b_scales,
    torch::Tensor marlin_workspace, torch::Tensor down_weight,
    torch::Tensor up_weight, bool use_packed_prefill,
    int64_t packed_prefill_config) {
...
TORCH_LIBRARY(gptqmodel_marlin_fp16, m) {
  ...
  m.def(
      "gptq_marlin_gemm_eora_fp16(Tensor a, Tensor? c, Tensor b_q_weight, Tensor? b_bias, "
  ...
  m.def(
      "gptq_marlin_gemm_eora_prepared_fp16(Tensor a, Tensor b_q_weight, Tensor b_scales, "
...
TORCH_LIBRARY_IMPL(gptqmodel_marlin_fp16, CUDA, m) {
  ...
  m.impl("gptq_marlin_gemm_eora_fp16",
         &gptq_marlin_gemm_eora_fp16_dispatch);
  m.impl("gptq_marlin_gemm_eora_prepared_fp16",
         &gptq_marlin_gemm_eora_prepared_fp16_dispatch);
