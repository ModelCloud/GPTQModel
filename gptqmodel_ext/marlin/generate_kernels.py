...
EORA_TEMPLATE = ("template __global__ void MarlinEoraRank128<"
                 "{{scalar_t}}, "
                 "vllm::kU4B8.id(), "
                 "{{s_type_id}}, "
...
def _eora_output_path(root: Path, dtype: str) -> Path:
    return root / f"kernel_{dtype}_eora_ku4b8.cu"
...
def _render_eora_kernel_file_text(dtype: str) -> str:
    scalar_t = "half" if dtype == "fp16" else "nv_bfloat16"
    s_type_id = "vllm::kFloat16.id()" if dtype == "fp16" else "vllm::kBFloat16.id()"
    ...
    lines = [EORA_FILE_HEAD, "", f"// Fused Marlin+EoRA rank-128 instantiation for dtype={dtype}", ""]
...
def build_expected_kernels(root: Path | None = None) -> dict[Path, str]:
    ...
    for dtype in DTYPES:
        ...
        expected[_eora_output_path(root, dtype)] = _render_eora_kernel_file_text(dtype)
