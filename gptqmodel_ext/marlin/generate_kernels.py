...
#define MARLIN_KERNEL_FUNCTION MarlinEoraRank{{rank}}
#define MARLIN_EORA_FUSED 1
#define MARLIN_EORA_RANK {{rank}}
#include "kernel.h"
#include "marlin_template.h"
...
EORA_TEMPLATE = ("template __global__ void MarlinEoraRank{{rank}}<"
                 "{{scalar_t}}, "
                 "vllm::kU4B8.id(), "
                 "{{s_type_id}}, "
                 "256, 1, 8, 8, true, pipe_stages, 8, false>"
                 "( MARLIN_EORA_KERNEL_PARAMS_FOR({{scalar_t}}) );")
...
def _eora_output_path(root: Path, dtype: str, rank: int) -> Path:
    rank_suffix = "" if rank == 128 else f"_r{rank}"
    return root / f"kernel_{dtype}_eora{rank_suffix}_ku4b8.cu"
...
def _render_eora_kernel_file_text(dtype: str, rank: int) -> str:
    scalar_t = "half" if dtype == "fp16" else "nv_bfloat16"
    s_type_id = "vllm::kFloat16.id()" if dtype == "fp16" else "vllm::kBFloat16.id()"
    template = jinja2.Template(EORA_TEMPLATE).render(
        scalar_t=scalar_t,
        s_type_id=s_type_id,
        rank=rank,
    )
    file_head = jinja2.Template(EORA_FILE_HEAD).render(rank=rank)
    lines = [file_head, "", f"// Fused Marlin+EoRA rank-{rank} instantiation for dtype={dtype}", ""]
...
def build_expected_kernels(root: Path | None = None) -> dict[Path, str]:
    ...
    for dtype in DTYPES:
        ...
        for rank in EORA_RANKS:
            expected[_eora_output_path(root, dtype, rank)] = _render_eora_kernel_file_text(dtype, rank)
