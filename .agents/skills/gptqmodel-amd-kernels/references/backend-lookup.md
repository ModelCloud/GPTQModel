...
The revision above is a lookup provenance record, not a permanent dependency pin. AITER's inspected
`flydsl_hgemm(a, b, ...)` accepts contiguous A `[M,K]`, B `[N,K]`, FP16/BF16 inputs, same-type or FP32 output, and
currently gates to gfx950. Its tuned dispatcher can select `flydsl`, `opus`, `asm`, `skinny`, `triton`, `torch`, or
`hipblaslt` per shape. Confirm these contracts against the revision actually installed.
