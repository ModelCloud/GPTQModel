...
2. Install the CPU torch stack and project dependencies:
   ...
   ```
   If another `gptqmodel` package is installed on the same machine and shadows the Ultra worktree, force the local repo to be imported by running with `PYTHONPATH=<repo-root>`:
   ```bash
   PYTHONPATH=/home/ubuntu/repos/GPT-QModel-Ultra python smoke_lazy_load.py
   ```
3. Verify `gptqmodel` is importable from the repo root and is the Ultra build:
   ```bash
   PYTHONPATH=/home/ubuntu/repos/GPT-QModel-Ultra python -c "import gptqmodel; print(gptqmodel.__version__); print(gptqmodel.__file__)"
   ```
4. The unit test `tests/test_stage_inputs_capture.py` requires `pytest`. If it is missing, install it:
   ```bash
