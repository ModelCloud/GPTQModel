...
* 09/18/2026 `main`: Experimental QVQ W3/P32 GSQ now stores FP32-trained attention reconstruction directly as BF16 from the fused Hadamard kernel while retaining its exact FP32 saved tensor and backward arithmetic. The isolated operator improves 1.20x--1.27x and canonical attention fitting improves 1.02x--1.06x on H100, with bit-for-bit model states and unchanged held-out gains. See [the configuration and promotion guide](docs/gsq.md).
* 09/18/2026 `main`: Experimental QVQ W3/P32 GSQ now emits selected legal P32 words inside the exact fused identity-Fisher screen, removing the remaining eager repack chain. The 8,192 x 2,048 candidate constructor improves another 1.28x (3.04x cumulatively), and canonical candidate wall time improves 1.070x--1.084x with bit-for-bit model states and unchanged held-out gains. See [the configuration and promotion guide](docs/gsq.md).
* 09/18/2026 `main`: Experimental QVQ W3/P32 GSQ now fuses exact identity-Fisher candidate shift decoding and scoring. The complete 8,192 x 2,048 candidate constructor is 2.37x faster on H100, and overlapped per-layer candidate wall time improves 1.13x--1.24x while preserving bit-for-bit model states, guards, changed-tile counts, held-out gains, and strict split disjointness. See [the configuration and promotion guide](docs/gsq.md).
* 09/18/2026 `main`: Experimental QVQ W3/P32 GSQ now derives legal three-way P32 overlap metadata inside its fused SM90 materializer. This removes 899 MiB of overlap-map allocation volume per Llama 3.2 1B layer and improves MLP fitting by 1.038x--1.054x while preserving bit-for-bit model states, guards, held-out losses, and strict split disjointness. See [the configuration and promotion guide](docs/gsq.md).
...
```python
from gptqmodel import GPTQModel, QuantizeEmbed, QuantizeEmbedConfig, RTNConfig
...
model.save("/path/of/Llama-3.2-1B-Instruct-RTN")
```
