#!/usr/bin/env python3
"""H100 experiment: native eight-column Tensor Core path versus packed-transition decode."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("benchmark_artifacts/qvq_lr_exp_n8_mma_h100.json"))
    parser.add_argument("--markdown-output", type=Path,
                        default=Path("benchmark_artifacts/qvq_lr_exp_n8_mma_h100.md"))
...
def main() -> None:
    ...
    candidate_op = load_kernel(
        name="qvq_lr_exp_n8_mma_ops",
        namespace="gptqmodel_qvq_exp_n8_mma",
        source="benchmark_artifacts/qvq_lr_exp_n8_mma.cu",
        build_root_env="GPTQMODEL_QVQ_LR_EXP_N8_MMA_BUILD_ROOT",
        default_build_root="/tmp/qvq-jit-current/qvq_lr_exp_n8_mma",
        display_name="QVQ native eight-column Tensor Core experiment",
    )
    torch.cuda.synchronize()
        ...
        for bits in args.bits:
            ...
            for m, x in inputs.items():
                ...
                for candidate, measured, metrics in (
                    ("last_packed", last_timing, last_error),
                    ("n8_mma_experiment", candidate_timing, candidate_error),
                ):
                    row = {
                        "shape": case.name,
                        "m": m,
                    ...
                    row["better_than_last"] = (
                        candidate == "n8_mma_experiment"
                        and measured["median_ms"] < last_timing["median_ms"]
                    )
                    rows.append(row)
                print(
                    f"complete {case.name} M{m} W{bits:g}: packed={last_timing['median_ms']:.4f}ms "
                    f"n8_mma={candidate_timing['median_ms']:.4f}ms "
    ...
    payload = {
        ...
        "last_source": "benchmark_artifacts/qvq_lr_exp_packed.cu",
        "candidate_source": "benchmark_artifacts/qvq_lr_exp_n8_mma.cu",
