...
#pragma unroll 1
    for (int u = 0; u < kBatchTiles; ++u) {
      if (u < tiles_here) {
        ...
        // four-transition sliding state across four adjacent pairs. The scalar
        // mapping performs four extractions for each of 16 states; this mapping
        // performs one extraction for each of the 16 unique transitions.
...
TORCH_LIBRARY_FRAGMENT(gptqmodel_qvq_proto, m) {
...
TORCH_LIBRARY_IMPL(gptqmodel_qvq_proto, CUDA, m) {
