#!/usr/bin/env python3
"""Isolated H100 test of storage-neutral cooperative W2/W2.5 local-ring decode."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("benchmark_artifacts/qvq_lr_cooperative_proto_h100.json"))
    parser.add_argument("--markdown-output", type=Path,
                        default=Path("benchmark_artifacts/qvq_lr_cooperative_proto_h100.md"))
...
def load_prototype():
    ...
    extension = TorchOpsJitExtension(
        ...
        sources=[str(REPO_ROOT / "benchmark_artifacts/qvq_lr_cooperative_proto.cu")],
        ...
        force_rebuild_env="GPTQMODEL_QVQ_LR_PROTO_FORCE_REBUILD",
...
def markdown(payload: dict) -> str:
    ...
    for row in payload["rows"]:
        lines.append(
            ...
            f"{row['logical_tflops']:.3f} | {row.get('speedup_vs_current', 1.0):.3f}x | "
...
def main() -> None:
    ...
    from gptqmodel.utils.qvq_cuda import _pgc16_levels, prewarm_qvq_cuda, qvq_cuda_gemv
    ...
    if not prewarm_qvq_cuda():
        raise RuntimeError("production QVQ extension failed to load")
    proto_op = load_prototype()
    torch.cuda.synchronize()
    ...
    for shape_index, case in enumerate(cases):
        ...
        for bits in args.bits:
            ...
            for m, x in inputs.items():
                ...
                torch.cuda.synchronize()
                current_error = error_metrics(torch, current_output, reference)
                if not current_error["finite"] or float(current_error["max_abs"]) > 2e-3:
                    raise AssertionError(f"current path correctness failure: {current_error}")
                proto_error = validate_repeated(
                    torch, proto_call, reference, repeats=args.correctness_repeats
                )
                current_timing = timing(
                    torch, current_call, warmup=args.warmup, iterations=args.iterations
                )
                proto_timing = timing(
                    torch, proto_call, warmup=args.warmup, iterations=args.iterations
                )
                logical_flops = 2 * m * case.in_features * case.out_features
                for candidate, measured, metrics in (
                    ("current", current_timing, current_error),
                    ("cooperative_proto", proto_timing, proto_error),
                ):
                    row = {
                        "shape": case.name,
                        "m": m,
                    ...
                    row["speedup_vs_current"] = current_timing["median_ms"] / measured["median_ms"]
                    ...
                    row["better_than_last"] = candidate == "cooperative_proto" and measured["median_ms"] < current_timing["median_ms"]
                    rows.append(row)
                print(
                    f"complete {case.name} M{m} W{bits:g}: current={current_timing['median_ms']:.4f}ms "
                    f"proto={proto_timing['median_ms']:.4f}ms speedup={current_timing['median_ms']/proto_timing['median_ms']:.3f}x "
                    f"max_abs={proto_error['max_abs']:.3g}",
                    flush=True,
                )
    ...
    payload = {
        ...
        "prototype_source": "benchmark_artifacts/qvq_lr_cooperative_proto.cu",
